...
- **Entity:** inherited credentials strip (`CLAUDE_CODE_MESSAGING_TOKEN` / `_SOCKET` and every other inherited `CLAUDE*` name under the R8 prefix rule, except persistent-environment names — Windows registry `Environment` value names, Unix `config.json` `claude_env_keep` — and always the 11-name identity floor measured on the Windows host)
...
- **Entity:** dependency and boundary policy (the `cargo deny` tokio and C-crate bans, the `cargo check` of sync crates without tokio, clippy `-D warnings`, the cargo-modules orphans gate per lib/bin target plus the on-demand `--acyclic` boundary review, the release build carrying `viola` only, SHA-pinned Actions / zizmor)
...
- **Surface:** library-only (workspace crates `viola-core`, `viola-pty`, `viola-channel`, `viola-state`, `viola-agent-claude`, `viola-mcp`, `viola-ui` at the unit level)
  ...
  - **Notes:** traced to arch excerpt, Workspace / Modules and Test-Relevant Conventions. The arch registers exactly one test-only member, `crates/viola-e2e` (`publish = false`), and lists every test artifact in its §Project directory structure. These tests run on all three CI OSes.
...
| Property-based | Invariant assertions over random inputs | Required. proptest 1.11.0 over all 7 parser surfaces named by the property-test trigger, with `proptest-regressions/` committed. cargo-fuzz 0.13.2 targets run as a time-boxed ubuntu nightly job, plus a corpus replay gate: today `viola_name` (the pre-parser seed target), then the same seven parsers as they land. |
...
| Mutation | Test-suite strength on changed code | Required (Founder Direction 1). cargo-mutants 27.1.0 runs `--in-diff` against the chunk diff through `--test-tool=nextest`. Any `missed` or `timeout` outcome, or more `unviable` than `caught` outcomes (`unviable-exceeds-caught`), makes harness `run` fail. |
...
| Supply chain (V9) | CI gates | §9 Lint stage (sync-crate `cargo check`, `scripts/orphans-check.sh`) + Supply-chain stage (cargo deny incl. the `fuzz/Cargo.lock` audit, sole-root tokio ban, ban probes, zizmor) + Release build stage (`scripts/release-check.sh`) + `nightly.yml` weekly advisories (root and fuzz lockfiles) |
...
- Command body: `scripts/agent-run.sh run [--unit|--integration|--e2e|--browser|--mutants|--coverage|--perf|--fuzz-replay|--all] [--filter <nextest-filterset>] [--leg <name>] [--local-live]`. `--leg` needs `--mutants` (clap `requires`), and its name must pass the session-id charset, else usage `invalid-leg`. `--coverage` and `--fuzz-replay` never join the default selection. The default is `--all`, which runs these steps in order:
  ...
  2. Fake-agent E2E: `cargo nextest run --workspace --features viola/fake-agent --profile ci -E 'binary(/^(path|tui|mcp|http|sse|cross|chaos|contract)_/)'`. Every nextest test uses one boot model:
     ...
     - `crates/viola-e2e` scenarios use one of two fixtures, chosen by the scenario's §6 Cleanup line:
       ...
       - Cleanup "TempDir drop" or a test-owned outer PTY (Paths 1, 5, E1, E2, E5): the chain `home → fake_agent_path → stamped_home → booted_wrapper` from `viola_e2e::fixtures`. Root `tests/support/` cannot be imported by another package, and the root package must not dev-depend on the tokio-based `viola-e2e`, so the chain exists twice (the sync root copy and the `viola-e2e` copy) against one contract. In both copies `stamped_home` runs `viola verify` against the fake agent. Today only the sync root copy exists (`tests/support/home.rs`; homes under `target/e2e-home/viola-test-*`). The `viola_e2e::fixtures` copy lands with its first consumer among these scenarios. Until `viola verify` exists, the root `stamped_home` is an interim seam (`stamped:false`, it writes no `ledger/stamps.json`), and `booted_wrapper` drives the wrapper through a test-owned outer PTY (`tests/support/outer_pty.rs`; `Wrapper::boot/send/stop` in `tests/support/home.rs`, and `tests/run_cli.rs::run_viola`) and waits for the interim readiness lines (`process-start` for `self` and `claude-child`) plus the fake agent's `start` receipt — its terminal is raw, so no key sent after it is swallowed — bounded at 10 s. The wait is exit-aware: when `child.try_wait()` reports that the wrapper exited first, it fails at once (`wrapper <name> exited before ready`) instead of waiting out the bound. The bound stays below cargo-mutants' 20 s auto-timeout floor, so a mutant that stops the wrapper from getting ready grades as caught, not Timeout. Harness `boot` keeps its 20 s per-instance bound.
...
- Exit code semantics: 0 only if every selected suite passes, there are no missed or timed-out mutants, and unviable mutants do not outnumber caught ones. Non-zero otherwise. With `--leg`, the mutants suite's survivors are deferred to the `gate` union. They still count in `survived` and `failures[]`, but only a tool-arm refusal or a non-survivor failure (`outcomes-missing`, `unviable-exceeds-caught`) makes the run red. A leg with `unviable > caught` is therefore red at its own run and never deferred (its `failed` exceeds its `survived`).
...
- **`gate --require <suite>[,<suite>…] [--artifacts <dir>] [--mutants-legs <leg>[,<leg>…]]`** (default dir `target/agent-run/artifacts/`):
  - `--require` takes only the nine closed `suite` values; an empty list or any other value (`a11y` included) is usage `unknown-suite`. A leg name outside the session-id charset is usage `invalid-leg`.
  - Inputs:
    ...
    - the `mutants` suite in `run-summary.json` (its `survived` = missed + timeout from `mutants.out/outcomes.json`), or, with `--mutants-legs`, one `mutants-verdict-<leg>.json` per named leg
  ...
  - A breach is any of:
    ...
    - single-leg: the `mutants` suite's `survived > 0`. With `--mutants-legs`, the union judges each mutant only by the named legs that COMPILE its line: `gate` reads the checked-out source (the same sha the legs ran on), parses the mutant's file with `syn`, and evaluates every `#[cfg]` on a node covering the line per leg family (`windows-` → windows; `ubuntu-` → unix + linux; `macos-` → unix + macos; `unix`, `windows`, `target_os`, `target_family`, `not`, `all`, `any` known). A leg is dropped only when a covering cfg is proven false for it; an unknown predicate or leg family, an unreadable name or file, or an unvisited node form keeps the leg. When no named leg compiles the line, every named leg judges it. A mutant is red when no judged leg reports `caught` and some judged leg reports `missed` or `timeout`: a mutant unviable on the leg that compiles its code is not a survivor whatever the other leg reads, and one missed on a compiling leg stays red (at the union, unviable on every judged leg is reported, not red; a leg whose own run had `unviable > caught` is already red at its `run --mutants --leg` step). A missing leg file is `artifact-missing`, and a partial union judges no mutant.
    - a required perf JSON that is missing, unreadable or whose `.results[0].max` is at or over its §10 gate. A missing artifact is a breach, never a pass.
  - Output: `{"v":1,"cmd":"gate","ok":<bool>,"breaches":[{"gate":"suite-missing"|"suite-failed"|"suite-skipped"|"coverage"|"mutants"|"perf"|"artifact-missing","suite":<name|null>,"detail":<string>}]}`. `detail` is a fixed code or a count, never file content: `absent`, `no-run-summary`, `failed N`, `skipped N`, `survived N`, `lines 84.99 < 85`, `<metric> unreadable`, `junit-<suite>.xml`, `llvm-cov-summary.json`, `mutants-verdict-<leg>.json`, `perf-*.json`, `<file> max X >= 1`, or a union's mutant name.
  - Exit 0 with no breaches, 1 on any breach, 2 on usage error.
  - CI runs `gate` as the last step of every test-running job, with `--require` set to exactly the suites that job ran (`playwright` in the ubuntu E2E job). The exception is the two `mutants (<os>)` legs: they only upload `mutants-verdict-<os>.json`, and the `mutants-verdict` job (ubuntu, `needs: mutants`, `if: always()`) runs `gate --require mutants --mutants-legs ubuntu-latest,windows-2025` over them, reading that job's own checkout of the same sha for the `#[cfg]`s. The per-job lists live in `ci.yml`. In the per-OS `test` job, `target/agent-run/` (including `artifacts/`, minus `target/agent-run/chunk.diff`) leaves CI only in obs-plan §9's scan-gated `harness-${{ matrix.os }}` artifact (`path` `target/agent-run/` plus `!target/agent-run/chunk.diff`; `if: failure() && steps.secret-scan.outcome == 'success'`). There is no unscanned `agent-run-<os>` upload.
- **Closed enums:**
  ...
  - `run --mutants` `mutants.verdict`: `counted`, `no-rust-delta`, `test-only-rust-delta`. Leg-verdict `outcome`: `caught`, `missed`, `timeout`, `unviable`.
  - `run` `reason` added by the quality-gates chunk: `fuzz-linux-only` (exit 2), `tool-missing`, `corpus-empty`. Usage `detail`: `invalid-leg`, `unknown-suite`.
...
- **ci-tool-install:** every other CI-invoked binary, version-pinned:
  ...
  - CI's log assertion G2 uses the runner-provided `jq`. It is presence-checked (`jq --version`) in the `test` job and never installed. `scripts/release-check.sh` (the `release` job) and `scripts/orphans-check.sh` (`lint`) use the same runner-provided `jq` and refuse with `tool-missing: jq` without it (runner images read at chunk 2026-09-24-workspace-tree-and-code-graph-planes: jq 1.8.1 on windows-2025, 1.8.2 on macos-15 arm64, 1.7 on ubuntu-24.04). jaq 3.1.1 remains a valid local assertion form (§3 `logs`).
...
- **quality-gate-config-emit:** emit `.github/workflows/ci.yml` enforcing:
  ...
  - `retries = 0`
  - hyperfine `max` gates
  - zero missed or timed-out mutants, and no leg with more unviable than caught mutants
...
- **viola-agent-claude:**
  ...
  - The R8 strip against a literal copy of the 11-name identity floor measured on the Windows host (`CLAUDECODE`, `CLAUDE_CODE_BRIDGE_SESSION_ID`, `CLAUDE_CODE_CHILD_SESSION`, `CLAUDE_CODE_ENTRYPOINT`, `CLAUDE_CODE_EXECPATH`, `CLAUDE_CODE_MESSAGING_SOCKET`, `CLAUDE_CODE_MESSAGING_TOKEN`, `CLAUDE_CODE_SESSION_ATTENDED`, `CLAUDE_CODE_SESSION_ID`, `CLAUDE_EFFORT`, `CLAUDE_PID`) written into the test, never imported from `IDENTITY_FLOOR`, plus the prefix rule: every inherited `CLAUDE*` name is removed (ASCII case-insensitive on Windows, exact on Unix) unless it is in the persistent set, and a floor name is removed whatever the persistent set says.
...
- **Steps:**
  1. Run `viola run builder` with `Command::env` set for the 11 identity-floor names plus an unknown `CLAUDE_VIOLA_TEST_NEW_X`. Values are secret-scan canaries (`canary-<kind>-value-<hex4>`, one per name, registered in the `claude-stripped` table), including `CLAUDE_CODE_MESSAGING_TOKEN` and `_SOCKET`.
  2. The fake agent dumps its env names to the receipt, and its open fds on Unix (`/proc/self/fd` on Linux, `/dev/fd` on macOS).
- **Verification signal:**
  - No stripped name appears in the receipt. On Unix a `claude_env_keep` name survives. `process-start{subject:"claude-child"}` carries `env_stripped_count` (≥ 12), `env_stripped_known` and `env_kept`, names only, and the secret scan finds 0 canaries. Landed as the strip half (`tests/tui_env_strip.rs`).
  - `VIOLA_NAME` and `VIOLA_DIR` are present — asserted when "Instance state and start order" sets them (its route CARRY).
  - On Unix, the fds are only 0/1/2 plus the PTY slave, so no channel handle is inherited — asserted when "Wrapper channel" lands the channel handle (its route CARRY). On Windows, handle non-inheritance is asserted at the product unit boundary.
...
- **Error sanitization and secret scan:** after every E2E test, the `events` support helper checks:
  ...
  - **Canary** (obs-plan §8, High-class verification): every integration and E2E input is synthetic, and every prompt, send text, tool `input`, dialog answer and statusline payload the tests feed embeds one fixed canary string, a constant in `tests/support`. The scan asserts that the canary never appears in any home-level `<home>/diagnostics/*.ndjson` line. It may appear in `events.ndjson` (contract payloads) and in `instances/<name>/diagnostics/detail-*.ndjson` only. The same scan runs in CI over `target/e2e-home/**`, the harness capture in `target/agent-run/` except the mutation leg's `target/agent-run/chunk.diff` (repository source text, excluded from the scan and from the `harness-<os>` upload), and `target/nextest/ci/junit.xml` before any upload (obs-plan §9 step order). A hit is reported as file, line, byte offset and pattern class, never the matched bytes.
...
- **Security control negatives** (`security_negatives_*.rs`; one negative test per security-plan control that had none):
  ...
  - **Windows client SQOS** (`#[cfg(windows)]`, client-side server verification). A test-owned same-user pipe server accepts the viola client's connection, calls `ImpersonateNamedPipeClient`, then reads the thread token's `TokenImpersonationLevel`. It must be `SecurityIdentification`, never `SecurityImpersonation` or higher, which proves the client opened with `SECURITY_SQOS_PRESENT | SECURITY_IDENTIFICATION`. The open recipe itself is already pinned, ahead of the client, by `tests/channel_sqos_open.rs` (`#[cfg(windows)]`, windows-2025 CI `test` leg): `sqos_identification_open_adopted_reads_identification` (the `CreateFileW` SQOS + `FILE_FLAG_OVERLAPPED` open adopted by `Stream::try_from` reads `SecurityIdentification` at a test-owned server on `\\.\pipe\viola-test-sqos-<pid>-<label>`) and its control `sqos_flags_absent_open_reads_impersonation` (no SQOS flags → `SecurityImpersonation`). The `security_negatives_*.rs` case against the viola client lands with `viola-channel`, its first consumer.
...
- **Chaos suite** (`chaos_*.rs`, sysinfo 0.39.6 + std):
  ...
  - the no-EOF fake mode: the wrapper detects exit on the handle (`exit_source:"handle-wait"`). Whether the output outlives the session leader is OS-specific, as measured in CI runs 36165685381 / 36166907442: natively on ConPTY; on Linux only while the `--exit-no-eof` holder runs in its own process group (a holder in the leader's foreground group dies by SIGHUP when the leader exits); on macOS the output ends at the leader's exit even with the holder alive in its own group. The held-output half is therefore asserted on Windows + Linux only, and on macOS exit on the handle is shown by the handle wait alone (`tests/tui_pty_seam.rs`) and a `session-end` record is logged
  ...
  - `.cmd` child (`#[cfg(windows)]`): `run` exits 1 — landed in `tests/cli_program_resolution.rs` (a `.cmd` and a `.bat` child each exit 1 with no child spawned, `process-exit{subject:"self", exit_code:1, detail:"batch-script-child"}`, exactly the two fixed `unable:`/`hint:` stderr lines; the same binary proves `claude.cmd` → sibling `claude.exe` resolution)
- **Property suite** (proptest 1.11.0, `cases: 512`):
  ...
  cargo-fuzz 0.13.2 targets live in the separate `fuzz/` cargo workspace and use shared `arbitrary` 1.4.2 inputs. The first is `viola_name` (`ViolaName::try_new` against an independent literal oracle, 18 synthetic seeds): an eighth, pre-parser seed target, so the pipeline runs with a real consumer. The seven parser targets join as their parsers land.
- **Contract suite** (`contract_*.rs`):
  ...
  - `tests/contract_content_hash.rs` pins the pinned-exe content hash (§1 "SHA-256 16-hex re-hash") to its library, `sha2` (a root dev-dependency): `sha256_published_vector_matches_digest` over the three FIPS 180-2 vectors and `sha256_truncated_to_eight_bytes_yields_sixteen_hex` over the 8-byte / 16-hex truncation, literal oracles, on all three OSes.
...
- It writes receipt lines to `--receipt <path>`: ndjson, one `write_all` per line, every line `"v":1` + a kebab `kind`. The kinds are:
  - `start` {`cli_version`}: written after the fake agent has taken its `HostTerminal` raw-mode guard on its own stdin/stdout (as the real CLI sets raw mode itself), so tests send no key before it
  - `size` {`cols`, `rows`}: at start and again whenever the terminal size changed (the resize oracle)
  - `cwd` {`cwd`}
  - `hold` {`pid`}: the `--exit-no-eof` holder, at its start
...
|-------|------|----------|-------|
| Lint | `cargo fmt --check`; `cargo clippy --workspace --all-targets --features fake-agent -- -D warnings`; `cargo check` with one `-p` per crate listed in `scripts/sync-crates.txt` (no `--features`, so no tokio; target set `viola-core`, `viola-pty`, `viola-state`, `viola-channel`, `viola-agent-claude`, each joining the list with its crate; an empty list fails; job `lint`); `bash scripts/orphans-check.sh --probe && bash scripts/orphans-check.sh` (cargo-modules 0.27.0: `cargo modules orphans -p <pkg> {--lib | --bin <name>} [--features <required features>] --deny` for every lib/bin target from `cargo metadata --no-deps`, last line `orphans-check: N/N targets clean`; the probe fires on a planted orphan, last line `orphans-check probes: 1/1 fired, control clean`). `cargo modules dependencies --acyclic` is an on-demand boundary review, not a gate: at cargo-modules 0.27.0 every type and its own inherent method read as a cycle whatever the filters (measured at chunk 2026-09-24-workspace-tree-and-code-graph-planes, 3 of 4 targets). The rmcp release-graph assertion (`cargo tree -e features -p viola --edges normal`, rmcp only `server`, `transport-io`) has no subject until `viola-mcp` exists and joins this row with the "MCP server for drivers" chunk. (The a11y lint needs `e2e-web/node_modules`, so it runs in the ubuntu browser job; see the E2E row.) | per OS matrix | Swatinem/rust-cache v2.9.2 (SHA-pinned; key: OS + toolchain + `Cargo.lock`), in `ci.yml` only |
| Supply-chain | ubuntu job `supply-chain`, fail-closed `shell: bash` steps: `cargo deny --format json check` (`deny.toml`: advisories, licences, sources, bans); `cargo deny --manifest-path fuzz/Cargo.toml --format json check advisories sources` into `target/supply-chain/deny-fuzz.json`, run from the repo root (under `fuzz/`, rustup would demand the fuzz nightly); the tokio ban once per crate in `scripts/sync-crates.txt` as sole root, `cargo deny --config deny-sync.toml --manifest-path crates/<crate>/Cargo.toml check bans`; `bash scripts/deny-probes.sh` (a permanent negative probe per ban + a clean control, last line `deny-probes: 13/13 banned, control clean`); `zizmor --format=json .github/workflows/`. The deny (root and fuzz) and zizmor JSON go to `target/supply-chain/`, uploaded `if: always()` as artifact `supply-chain` (7 days; admissible by content, obs-plan §8 item 6). Weekly: `nightly.yml` job `advisories` runs `cargo deny check advisories` and `cargo deny --manifest-path fuzz/Cargo.toml check advisories` on `schedule` + `workflow_dispatch` | single job (+ the weekly job) | rust-cache (`ci.yml` job only; `nightly.yml` none) |
...
| Coverage report | `agent-run run --coverage` (cargo-llvm-cov 0.9.1 under nextest). In CI this is the per-OS test job. Its one instrumented nextest run replaces the Unit, Integration and `run --e2e` rows, which are the local `agent-run` equivalents, and the doctest run follows it. Each job's last step is `viola-harness gate --require <suites>`: the per-OS test job uses `coverage,doctest`, the ubuntu E2E job (`run --browser`) uses `playwright`, the Perf, Fuzz replay and MSRV jobs use `perf`, `fuzz-replay` and `nextest-unit`, and the Mutation gate runs in the `mutants-verdict` job as `gate --require mutants --mutants-legs ubuntu-latest,windows-2025` | per OS | rust-cache |
| Mutation | `mutants (<os>)`, a two-leg matrix (`ubuntu-latest`, `windows-2025`, `fail-fast: false`) on push and pull_request: `agent-run run --mutants --leg "$LEG"` (step "Mutation leg (whole chunk)") with only `LEG` through `env:` and no `github.event` value: the harness derives the chunk base (§3 `run` step 4 Base), so every push of an operator pass mutates the whole chunk; each leg uploads only `mutants-verdict-<os>.json`. The ubuntu `mutants-verdict` job (`needs: mutants`, `if: always()`, actions/download-artifact v8.0.1 `merge-multiple`) runs the union gate over its own checkout of the same sha: each mutant is judged only by the legs whose `#[cfg]`s compile its line, or by every leg when none does, and is red when no judged leg caught it and some judged leg missed it or timed out (cargo-mutants reports another OS's `#[cfg]` bodies as missed). Before the union, a leg whose own run had `unviable > caught` is red and is not deferred | two legs + one verdict job | rust-cache (legs) |
...
| MSRV | ubuntu `msrv`: `rustup toolchain install 1.96 --profile minimal`, then `RUSTUP_TOOLCHAIN=1.96` steps for `rustc --version`, `cargo check --workspace` and `agent-run run --unit`, then `gate --require nextest-unit`; no toolchain action (`RUSTUP_TOOLCHAIN` outranks `rust-toolchain.toml`, as measured at chunk 2026-09-24-quality-gates, research.md M4). 1.96 is the security toolchain floor and the workspace `rust-version` requested from arch (obs-plan D-22; sysinfo 0.39.6 needs 1.95) | single job | rust-cache (`key: msrv`) |
| Fuzz replay | ci.yml `fuzz-replay` (ubuntu, push and PR): `rustup toolchain install` (root, then `working-directory: fuzz`), `cargo install --locked cargo-fuzz@0.13.2`, `agent-run run --fuzz-replay` (`-runs=0` corpus replay), then `gate --require fuzz-replay`. `nightly.yml` job `fuzz` runs `cargo +<channel> fuzz run --fuzz-dir fuzz <t> fuzz/corpus/<t> -- -max_total_time=120` per target (channel from `fuzz/rust-toolchain.toml`), uploading `fuzz/artifacts/` on `failure()`; no a11y step | single job (+ the nightly job) | none |
| Release build | ci.yml job `release` (`fail-fast: false`): `bash scripts/release-check.sh --probe` (last line `release-check probes: 3/3 refused, control clean`), then `bash scripts/release-check.sh`, which runs `cargo build --release --locked --bin viola --message-format=json` (fake agent excluded: feature off) and judges that build's own `compiler-artifact` executables. It prints `release-check: viola only` or fails on a test-only binary or on no executable, and never lists `target/release/`. A plain exit-code job: no upload, no `viola-harness gate` step | per OS | rust-cache (ci.yml only; never in a release workflow) |
| Quality gates | `viola-harness gate`, which reads `junit-*.xml`, `run-summary.json`, the per-leg `mutants-verdict-<leg>.json` files, the llvm-cov JSON summary and the hyperfine JSON, and exits non-zero on any §10 breach | n/a | n/a |

Every workflow (`ci.yml`, `nightly.yml`) has `permissions: {}` at the top and `contents: read` per job. Every `uses:` is SHA-pinned; zizmor `unpinned-uses` is the assertion. Tools are installed with taiki-e/install-action v2.87.19 (SHA-pinned) as `tool: cargo-nextest@0.9.146,cargo-llvm-cov@0.9.1,cargo-mutants@27.1.0,cargo-deny@0.20.2`. hyperfine 1.20.0, cargo-modules 0.27.0 and zizmor 1.30.1 are installed with `cargo install --locked <crate>@<version>` in the jobs that invoke them. G2 (`test`), `scripts/orphans-check.sh` (`lint`) and `scripts/release-check.sh` (`release`) use the runner-provided `jq`, never installed: G2 presence-checks it (`jq --version`) and the two scripts refuse with `tool-missing: jq`. ripgrep 15.2.0, the G1/G3 tool, comes from `scripts/install-ripgrep.sh` (pinned, checksum-verified official release) in the `lint` job. cargo-fuzz 0.13.2 is installed with `cargo install --locked`, only in the `fuzz-replay` and nightly `fuzz` jobs, with the channel from `fuzz/rust-toolchain.toml`. See §3 Bootstrap `ci-tool-install`. rustfmt and clippy come from `rust-toolchain.toml` `components`, installed by the `rustup toolchain install` step (no toolchain action; the MSRV and fuzz jobs, which need a different toolchain, name theirs). cargo-deny and zizmor run as the pinned CLIs above; cargo-deny-action and zizmor-action are not used, so each tool has exactly one version source. cargo-deny 0.20 takes `--config <path>` as a global option before the subcommand; `check -c` is rejected (as measured on cargo-deny 0.20.2, dev host, 2026-09-24).

**Matrix builds** (from the multi-os-compat trigger):

- OS: `windows-2025` (ConPTY, named pipe, DACL / SDDL read-back, `.cmd` refusal), `macos-latest` (macOS 26 arm64: openpty, Unix socket, euid + dir-only server verification; pin `macos-26` if the label moves), `ubuntu-latest` (openpty, Unix socket, modes; also the Playwright, deny, zizmor, `mutants-verdict`, fuzz and MSRV jobs). The mutation job runs a leg on both `ubuntu-latest` and `windows-2025`. `fail-fast: false`.
...
- In the per-OS `test` job, every test artifact leaves CI only behind obs-plan §9's secret scan (`id: secret-scan`):
  - `target/agent-run/` minus `target/agent-run/chunk.diff` (the mutation leg's diff, which the scan skips and the upload excludes), including `artifacts/` with the per-suite `junit-<suite>.xml`, `run-summary.json`, `llvm-cov-summary.json` and `perf-<hook>.json` (§3 `gate`) and the failure-only `logs.ndjson` / `status.json` capture: `harness-${{ matrix.os }}`, on failure with a passed scan;
  - `target/e2e-home/**/diagnostics/*.ndjson`: `diag-${{ matrix.os }}`;
  - `target/nextest/ci/junit.xml`, the report of the job's single instrumented `run --coverage` nextest invocation: `junit-${{ matrix.os }}`;
...
- each mutants leg's `mutants-verdict-<os>.json` (repo-relative mutant names and outcomes only). `mutants.out/` is never uploaded: its `outcomes.json` holds absolute argv paths and its `log/` holds test output.
...
- Any missed or timed-out mutant in the chunk diff, or a mutation leg with more unviable than caught mutants
- A hyperfine budget breach, or a fuzz corpus replay crash
- Lint errors: fmt, clippy `-D warnings`, a sync crate listed in `scripts/sync-crates.txt` failing to build without tokio (or an empty list), a cargo-modules orphan in any lib/bin target (`scripts/orphans-check.sh` not ending `N/N targets clean`, or its probe not ending `1/1 fired, control clean`), or an eslint-plugin-lit-a11y / html-validate finding in the ubuntu a11y lint
- Supply-chain errors: a cargo deny ban/advisory/licence/source finding, a `fuzz/Cargo.lock` advisory or source finding, a listed sync crate failing the sole-root `deny-sync.toml` tokio ban, `scripts/deny-probes.sh` not ending `deny-probes: 13/13 banned, control clean`, a zizmor finding, or the weekly `nightly.yml` advisories run (root and fuzz lockfiles) failing
- Release-build errors: `scripts/release-check.sh` not ending `release-check: viola only`, or its probe not ending `release-check probes: 3/3 refused, control clean`
...
- Excluded via the separator-agnostic `--ignore-filename-regex` `(viola-fake-agent|crates[/\\]viola-e2e|tests[/\\]support|fuzz[/\\])`, the harness const `COVERAGE_IGNORE`. llvm-cov reports Windows paths with `\`, so the forward-slash form never matched there (as measured at chunk 2026-09-24-quality-gates, research.md M5).
...
**Mutation gate (Founder Direction 1):** `mutants.out/outcomes.json` for the chunk diff must show `missed == 0`, `timeout == 0` and `unviable <= caught`. `unviable` mutants are reported, and a few do not fail the gate. When they outnumber the caught ones the run is red (`unviable-exceeds-caught`), because a run whose mutant builds mostly failed is no evidence of test strength. The threshold was picked by measurement across every leg with an uploaded verdict (2026-09-25). Healthy legs had unviable/caught ≤ 0.044 (chunk 7: 3/114 and 3/113; this cleanup: 4/139, 4/139, and 6/137 on the dev host). The failed windows leg of run 36118112104 had 135/8. A diff that names no `.rs` path (docs, specs, fixtures) never reaches cargo-mutants: it passes with the explicit `"mutants":{"tested":0,"verdict":"no-rust-delta","diff":"target/agent-run/chunk.diff","files":N}`. A diff whose `.rs` paths are all test targets (`tests/`, `benches/`, `examples/`, at the root or under `crates/<member>/`) has nothing cargo-mutants can mutate: it passes only with the explicit `test-only-rust-delta` verdict, which names the diff, its file count and `rust_files`. Any other diff naming a `.rs` path, a mixed one included, is `counted`, and a counted run that produces no fresh `outcomes.json` is red. A mutant caught only by a doctest counts as missed under nextest, so the missing unit test must be added. In CI the verdict is the union of two legs, `ubuntu-latest` and `windows-2025`: each writes `mutants-verdict-<leg>.json` (`run --mutants --leg`), and the `mutants-verdict` job judges each mutant only by the legs that compile its line (read from the checked-out source's `#[cfg]`s with `syn`; a leg is dropped only when a cfg is proven false for it), or by every leg when none does, and fails it when no judged leg caught it and some judged leg missed it or timed out. cargo-mutants does not understand conditional compilation and reports another OS's `#[cfg]` bodies as missed (mutants.rs/limitations), so a `#[cfg(windows)]` / `#[cfg(not(unix))]` body is killed, or proven unviable, on the leg that compiles it: a mutant unviable on its compiling leg is not a survivor whatever the other leg reads, and one missed on its compiling leg stays red. Witness: the union replayed over run 36165685381's two recorded leg files (186 mutants each) reads `ok:true`, where the previous union printed exactly the `HostTerminal::enter -> Some(Default::default())` pair at `crates/viola-pty/src/lib.rs:395:9` / `:420:9` (as measured at chunk 2026-09-26-ci-chunk-base-and-union-verdict). A macOS-only (`target_os = "macos"`) body has no leg yet, so every leg judges it.
...
- Any missed or timed-out mutant, or unviable mutants outnumbering caught ones (`unviable-exceeds-caught`)
- Lint / clippy / cargo-deny / zizmor / cargo-modules orphans failure, or a release build carrying a test-only binary
...
- NEVER write brittle assertions against fixture-internal values; assert
  on contract
- NEVER import the product's own `IDENTITY_FLOOR`, ledger rows or refusal ordering as the test oracle. The expected values (the 11 identity-floor names, the refusal order, exit codes) are written as literals in the test.
...
- NEVER serialise tests outside the nextest `fixed-port` group. Isolation comes from the per-test home. The fuzz stages are single jobs, and mutation is a fixed two-leg OS matrix plus one `mutants-verdict` job, by design (§9). That does not license `max-threads = 1` anywhere else.
...
- NEVER widen the coverage `--ignore-filename-regex` (`viola-fake-agent|crates[/\\]viola-e2e|tests[/\\]support|fuzz[/\\]`) to pass a gate. The separator class that makes it match Windows paths is not a widening, because the four trees are unchanged. It is a back door to lowering the §10 thresholds and needs a Decisions Log entry first.
...
- NEVER trust the cargo-mutants exit code alone. Exit 3 masks exit 2, so count `missed`, `timeout`, `unviable` and `caught` in `outcomes.json`: any missed or timed-out mutant, or more unviable than caught, is red.
