...
- **Decision:**
  ...
  - New closed values: `reason:"usage"` with `detail` ∈ {`invalid-session-id`, `session-exists`, `invalid-instance`, `arguments`, `no-supervise-spec`}, the cleanup field `processes_gone`, the status member `instances`.
