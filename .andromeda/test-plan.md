...
|-------|------|----------|-------|
| Lint | `cargo fmt --check`; `cargo clippy --workspace --all-targets --features fake-agent -- -D warnings`; `cargo check -p viola-core -p viola-pty -p viola-state -p viola-channel -p viola-agent-claude` (no tokio); `cargo tree -e features -p viola --edges normal` (assert rmcp shows only `server`, `transport-io`); `cargo modules dependencies --package <crate> --acyclic` and `cargo modules orphans --package <crate> --deny` per crate. On ubuntu only: `cargo deny --format json check` and `zizmor --format=json .github/workflows/`. (The a11y lint needs `e2e-web/node_modules`, so it runs in the ubuntu browser job; see the E2E row.) | per OS matrix | Swatinem/rust-cache v2.9.2 (SHA-pinned; key: OS + toolchain + `Cargo.lock`), in `ci.yml` only |
