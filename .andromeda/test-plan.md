...
- Exit code semantics: 0 only if every selected suite passes and there are no missed or timed-out mutants. Non-zero otherwise. With `--leg`, the mutants suite's survivors are deferred to the `gate` union. They still count in `survived` and `failures[]`, but only a tool-arm refusal or a non-survivor failure (`outcomes-missing`) makes the run red.
...
- **`gate --require <suite>[,<suite>…] [--artifacts <dir>] [--mutants-legs <leg>[,<leg>…]]`** (default dir `target/agent-run/artifacts/`):
  ...
  - A breach is any of:
    ...
    - single-leg: the `mutants` suite's `survived > 0`. With `--mutants-legs`, the union: a mutant name no leg reports `caught` while some leg reports `missed` or `timeout` (unviable on every leg is reported, not red). A missing leg file is `artifact-missing`, and a partial union judges no mutant.
