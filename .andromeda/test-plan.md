...
- Command body: `scripts/agent-run.sh run [--unit|--integration|--e2e|--browser|--mutants|--coverage|--perf|--fuzz-replay|--all] [--filter <nextest-filterset>] [--leg <name>] [--local-live]`. `--leg` needs `--mutants` (clap `requires`), and its name must pass the session-id charset, else usage `invalid-leg`. `--coverage` and `--fuzz-replay` never join the default selection. The default is `--all`, which runs these steps in order:
  ...
  4. Mutation:
     - Base: `AGENT_RUN_CHUNK_BASE`, or else `git merge-base HEAD origin/main` (a fallback that never resolves on this project's single build branch). Locally /implement sets `AGENT_RUN_CHUNK_BASE` to the chunk's starting commit. In CI the `mutants` job runs on push and pull_request, sets `AGENT_RUN_CHUNK_BASE` through `env:` to the PR base sha or else `github.event.before`, and checks out with `actions/checkout` `fetch-depth: 0`, because the default shallow clone has no base commit. If the base commit is not reachable (`git cat-file -e <base>^{commit}` fails), `run --mutants` exits 1 with `reason:"base-missing"`. A force-push reaches this state, as measured on CI run 36117447745 (2026-09-25). `github.event.before` is then the replaced commit, which no ref reaches, so even a full-history checkout lacks it. Rewinding the branch to the chunk base and fast-forwarding from there makes `before` the base again. It never writes an empty `chunk.diff`, which cargo-mutants would report as zero mutants, i.e. a false `"mutants":{"tested":0}` pass.
...
- **`gate --require <suite>[,<suite>…] [--artifacts <dir>] [--mutants-legs <leg>[,<leg>…]]`** (default dir `target/agent-run/artifacts/`):
  ...
  - A breach is any of:
    ...
    - single-leg: the `mutants` suite's `survived > 0`. With `--mutants-legs`, the union: a mutant name no leg reports `caught` while some leg reports `missed` or `timeout` (at the union, unviable on every leg is reported, not red; a leg whose own run had `unviable > caught` is already red at its `run --mutants --leg` step). A missing leg file is `artifact-missing`, and a partial union judges no mutant.
...
| Mutation | `mutants (<os>)`, a two-leg matrix (`ubuntu-latest`, `windows-2025`, `fail-fast: false`) on push and pull_request: `agent-run run --mutants --leg "$LEG"` with `AGENT_RUN_CHUNK_BASE` = `github.event.pull_request.base.sha` or else `github.event.before` and `LEG`, both through `env:`; each leg uploads only `mutants-verdict-<os>.json`. The ubuntu `mutants-verdict` job (`needs: mutants`, `if: always()`, actions/download-artifact v8.0.1 `merge-multiple`) runs the union gate: a mutant is red only when no leg caught it (cargo-mutants reports another OS's `#[cfg]` bodies as missed). Before the union, a leg whose own run had `unviable > caught` is red and is not deferred | two legs + one verdict job | rust-cache (legs) |
