...
- Exit code: 0 on ready within the timeout. Otherwise it exits 1 with `{"v":1,"cmd":"boot","ok":false,"reason":"build-failed"|"verify-failed"|"run-exited"|"readiness-timeout"|"ui-not-ready"|"ui-port-taken"|"stale-embedded-assets", "instance":<name|null>, "exit_code":<int|null>, "missing":[<unmet readiness checks>]}`. When the reason is `run-exited`, `exit_code` is the wrapper's exit code verbatim (for example 1 for a live endpoint or `stale` heartbeat).
