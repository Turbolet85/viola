...
- **Steps:**
  ...
  7. Per-instance override: boot a second wrapper `overseer` in the same home, with the budget still over threshold (`five_hour` 91). Run `viola release builder --budget --json` only. Read `/api/sessions` and `viola list --json`, then `send` to both instances. Advance the injected clock (mock_instant) past `budget_override_until` and `send` to `builder` again.
