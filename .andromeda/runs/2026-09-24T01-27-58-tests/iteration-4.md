...
- Command body: `scripts/agent-run.sh boot [--session <id>] [--instance <name>[:<fake-agent-args>]]... [--ui] [--unstamped] [--cli-version <ver>] [--agents-mode recorded|oversize|malformed]`. The default is instances `overseer` and `builder` plus `--ui`, with `--agents-mode recorded`. `--agents-mode` sets `FAKE_CLAUDE_AGENTS_MODE` (§7) via `Command::env` on every child the supervisor spawns and on the `viola list` that `status` runs. It is recorded in `session.json` and `supervise.json`, so a spec can reproduce the E1 unwrapped row. Steps:
  ...
  5. Spawn the detached supervisor `viola-harness supervise --session <id>`. It owns one portable-pty (`=0.8.1`, via the `viola-pty` seam) outer PTY per instance and runs `viola run <name> --home <home> -- <CARGO_BIN_EXE_viola-fake-agent> --cli-version <ver> --fixtures fixtures/claude/<ver> --receipt <home>/fake/<name>.receipt.ndjson <fake-agent-args>`. It drains every master on its own thread.
...
- **Steps:**
  1. `agent-run boot --session p7 --unstamped --instance builder --ui`. Separately, boot a stamped home whose fake agent reports `--cli-version 0.0.0-unlisted`.
