...
- **File naming:**
  ...
  - Fixtures are named `<hook-event>.<variant>.json`, for example `pre-tool-use.ask-question.json`.
...
| Supply chain (V9) | CI gates | §9 Lint stage |
...
- **Exit codes:** every harness command exits 0 on success, 1 on failure and 2 on usage error. Each prints exactly one JSON document on stdout, starting `{"v":1,"cmd":"<command>","ok":<bool>,…}`.
...
- Readiness signal. `boot` polls with a bounded 100 ms interval against a deadline; this is a file-state probe, not a sleep-based synchronisation. Per instance, all of the following must hold:
...
- Command body: `scripts/agent-run.sh run [--unit|--integration|--e2e|--browser|--mutants|--coverage|--perf|--fuzz-replay|--all] [--filter <nextest-filterset>] [--local-live]`. The default is `--all`, which runs these steps in order:
  1. Unit + integration: `cargo nextest run --workspace --features fake-agent --profile ci -E '<layer filterset>'`, then `cargo test --workspace --doc`. The layer filtersets are:
  ...
  4. Mutation:
     ...
     - Diff: `git diff <base>...HEAD` written to `target/agent-run/chunk.diff`.
     - Command: `NEXTEST_PROFILE=mutants cargo mutants --workspace --features fake-agent --in-diff target/agent-run/chunk.diff --test-tool=nextest`.
...
- **Steps:**
  ...
  3. The fake agent fires `pre-tool-use.ask-question.json`. Run `viola answer builder <id> --json`, and rmcp `answer`.
  4. `GET /api/sessions`.
...
| Recorded hook payloads | Checked-in `fixtures/claude/<cli-version>/<hook-event>.<variant>.json`. Recorded locally by live `viola verify` (Haiku), then scrubbed and committed. CI replays them. | per-suite, read-only |
...
- It reads the hook commands from the `plugin/` and `settings.json` files that `run` wrote, never from PATH, and invokes them with fixture payloads on stdin.
- It accepts bracketed pastes + Enter as one prompt and fires UserPromptSubmit with the fixture-normalised text.
...
**Fixture hygiene:**
- One rstest `#[files("fixtures/claude/*/*.json")]` walk does two checks per fixture:
...
- **Key decisions:**
  ...
  - **Test crate deviation:** a test-only crate `crates/viola-e2e` (publish = false) is added. It hosts `viola-harness` and the tokio-based test clients (rmcp client, reqwest, eventsource-client) and is added to the cargo-deny tokio wrappers list. This keeps the sync root package and sync crates free of tokio dev-deps, as research's cargo-deny key detail offers. The arch statement "No separate test crate is declared" needs updating. Its tests find `viola` / `viola-fake-agent` through assert_cmd's target-dir lookup after the harness `cargo build --workspace --features fake-agent`.
