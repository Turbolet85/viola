...
| 1.4.1 Use of Color | yes | state-word assertions per state (`read back`, `unable · <reason> · <detail>`, `stale`, `DIALOG <kind>`, `expired`, `unconfirmable`, `human`, `unverified-cli`, `budget-paused`) + `forcedColors: 'active'` checks; axe `link-in-text-block` |
