...
| State strips (first reading, empty rack/tape, 503/404/405, 401 access strip, `TAPE stopped`) | rack errors (503/404/405): a native `<p>` that is the next sibling after the affected rack's `<table>` inside `<main>`; 401 access strip: a native `<p>` that is the next sibling after the WRAPPED rack's `<table>` (rack position). Neither is ever inside `<table>` / `<caption>` / `<tbody>`, so reading order is rack heading (caption) → column headers → strip. `TAPE stopped`: a native `<p>` that is the first child of the `role="log"` `<div>`, before the trim notice and the `<ol>` (never an `<ol>` child, so the list holds only `<li>` event lines). html-validate content-model rules on the rendered DOM and the `ariaSnapshotJSON` region-order comparison (SC 1.3.2, 3.2.3) pin these positions; empty/first-reading rack lines are a `<tr>` with one `<td colspan="6">`; no role, announced through the status announcer (Section 7) | printed words; `--rule-info` box | none (no in-page recovery control) | 1.3.1, 1.4.1, 1.4.11, 3.3.1, 3.3.3, 4.1.3 |
