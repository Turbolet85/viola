...
- `/api/sessions` item: `{name, wrapped, liveness, status, wheel?, budget_paused?, dialog_pending, cli_version?, cli_verified?}`.
  ...
  - The envelope also carries a top-level `budget`: `{five_hour, seven_day, read_at, paused}`, or `"unknown"`. `five_hour` and `seven_day` are each `{used_percentage, resets_at}`, with either field `"unknown"` when unparseable and the whole window `"unknown"` when the statusline omits it, because the two windows reset independently and a budget pause lifts at the crossed window's own `resets_at`. `budget.json` holds the same per-window reading plus `read_at`. The page derives the reading's age from `read_at`.
...
`hook.dialog`'s `data` is the same object as its dialog event's `data`, without `dialog_id`.
...
- `instances/<ViolaName>/events.ndjson`, `events.ndjson.lock`, `snapshot.json`, `snapshot.json.lock`, `heartbeat`, `settings.json` (the per-session settings override that wraps the statusline), `diagnostics/` (reserved for `hook` and wrapper diagnostics; file format owned by obs). v1 never rotates or truncates `events.ndjson`, because `wait`'s `after`, `send`'s `cursor` and SSE ids are byte offsets into it. Bounding its size is an open item, and any scheme must keep existing offsets valid.
