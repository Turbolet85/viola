...
[Iteration 6] [substantive] Placed serde, serde_json, chrono and thiserror as workspace-shared dependencies, with chrono listed directly by the sync crates.
[Iteration 7] [substantive] Added a plugin-precedence capability-ledger row.
[Iteration 8] [substantive] A `link` event is appended only when the driver → driven pair is new, which bounds log growth and gives `since` one meaning.
[Iteration 9] [substantive] `events.ndjson` is never rotated or truncated in v1, because byte-offset cursors depend on it. Size bounding is a named open item that must keep offsets valid.
[Iteration 10] [substantive] Named `viola run` as the writer of `instances/<ViolaName>/settings.json` and stated that it reaches the child through the settings-override mechanism recorded in the capability ledger.
[Iteration 11] [substantive] Named `hook statusline` as the only writer of `budget.json`, and the wrapper's budget gate, `list` and `ui` as its readers.
[Iteration 12] [substantive] Workspace resolver changed from "2" to "3" (the edition 2024 default, MSRV-aware against `rust-version = "1.89"`).
[Iteration 13] [substantive] `/api/sessions` item: `cli_verified` is now optional. For unwrapped rows, `name` is defined (the `claude agents --json` session identifier, not a `ViolaName` and never a valid `target`), `liveness` and `dialog_pending` get fixed values, and the wrapped-only fields are absent.
[Iteration 14] [substantive] Fixed the order in which simultaneous refusals are reported for `send` and `answer`.
[Iteration 15] [substantive] Defined the MCP success result: `isError: false` with the channel `ok` payload as `structuredContent`.
