...
**Instance snapshot** (`instances/<ViolaName>/snapshot.json`, written only by that instance's wrapper, rewritten on every change to a field below): `data` is `{endpoint, pid, started_at, pinned_bin, statusline_command?, cli_version?, cli_verified, wheel, budget_paused, budget_override_until?, pending_dialog?, links}`.
...
- `prompt-submitted`: `{text, origin}`. `text` is the submitted prompt, which delivery matching compares after reversing tag escaping. `origin` is `driver` · `human` · `harness`.
