...
- `send` `{text, from?}` → `{submitted_at}` when confirmed, or `{confirmed:false, detail:"unconfirmable"}`.
- `wait` `{timeout_ms?}` → `{event:<ndjson event line>}` or `{timed_out:true}`.
