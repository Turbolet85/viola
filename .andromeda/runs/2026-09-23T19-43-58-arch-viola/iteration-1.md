...
- **Channel methods:** lower-case, dot-namespaced for hook traffic. Requests: `send`, `wait`, `last`, `list`, `answer`, `pause`, `release`, `link`, `unlink`, `hook.dialog`. Notifications: `hook.event`. JSON-RPC ids are integers, monotonic per connection.
...
- **`RefusalReason` (serialised kebab-case):** `human-typing` · `budget-paused` · `unverified-cli` · `not-delivered` · `unknown`. `unknown` is `#[serde(other)]`, so an older client can still read a refusal from a newer wrapper. `detail` values are kebab-case strings, for example `input-not-ready` and `no-prompt-submitted`. `unconfirmable` is not a refusal detail. It appears only in an `ok` payload (`{"confirmed":false,"detail":"unconfirmable"}`), for a ledger-listed local command whose post-condition is "none" or not yet measured on this CLI version.
- **CLI exit codes:**
  - `0` ok
...
- **Normalised event kinds (`viola-core`):** `turn-ended`, `prompt-submitted`, `question`, `permission`, `plan`, `session-end`, `link`, `unlink`. New kinds are added only in `viola-core`, never with Claude-specific names.
...
{"jsonrpc":"2.0","id":8,"result":{"ok":{"confirmed":false,"detail":"unconfirmable"}}}
...
- `VIOLA_BIN`: the absolute path of the pinned copy `<viola home>/bin/<version>-<hash>/viola(.exe)` (forward slashes), set by `viola run` in the child.
