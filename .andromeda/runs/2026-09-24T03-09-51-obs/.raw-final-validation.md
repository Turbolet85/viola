...
I read all of `D:/dev/projects/viola/.andromeda/runs/2026-09-24T03-09-51-obs/obs-plan-draft.md` (1742 lines) and checked it against `obs-scope.md`, `obs-research.md`, `upstream-context.md` and `review-feedback-1.md` in the same folder. The plan is in good shape: I found 7 issues, one of them medium severity, and none that break the structure.

**Checks that passed:**
- **Section 1:** it is a faithful copy of obs-scope.md. A line-by-line diff differs only in headings.
- **Tier:** Standard. All 12 sections are present. The Compliance Trace Fields subsection is correctly left out, because security is Minimal with no compliance triggers.
- **Section 3 harness contract:** all 8 items have concrete bodies, with no "TBD" and no `{placeholder}` text.
- **Log format:** the Section 3 schema matches upstream-context Section 5 → Test Harness Contract Summary word for word, and Section 6 points back to it.
- **Must-trace paths:** all 7 obs-scope paths have a `#### Scenario:` heading with spans, span attributes, log fields and cleanup filled in.
- **Triggers:** all 19 obs-scope triggers appear in the Section 2 coverage map.
- **Surfaces:** all 6 obs-scope surfaces have a row in the Section 4 surface table. Instrumentable crates have coverage, boundary-only entities are handled at the boundary, and not-instrumentable entities are excluded.
- **Library catalog:** every named library and version is in obs-research.md. That covers tracing, tracing-subscriber, chrono, opentelemetry and its bridge, the rejected OTel exporters, the rejected browser SDK, axum-tracing-opentelemetry, sentry, tracing-panic, human-panic, thiserror, anyhow, veil, redact, logroller, tracing-appender, json-subscriber, `metrics`, web-vitals, hyperfine, nextest, upload-artifact, sysinfo, rmcp and tower-http.
- **Anti-patterns:** Section 11 has all 9 subsections.
- **Zero unlogged panics:** Section 10 states the invariant explicitly, and the Section 11 SLO subsection bans retry-once. The two do not contradict each other.
- **Human-review wording:** no "manually verify", "developer reviews" or similar gating language. Datadog, Grafana and Kibana are named only in the "never use" context.

**Not flagged, but worth knowing:**
- The retry-once ban sits in Section 11 SLO, not in Universal. I did not copy it into Universal, because iteration 3 deliberately removed a duplicate Universal ban.
- The Universal subsection bans reading service identity from env vars, rather than the template's "no hardcoded service identity". This reversal is deliberate: the architecture fixes the name as `"viola"` and says env vars are not a configuration channel.
