...
[Iteration 9] [substantive] Fixed the Semantic Success row so the `read back` word is buff, not graphite on anthracite. Tokenised the readback strike width and exempted the two media-query breakpoints in the Token Test. Resolved the `DIALOG` 600-weight contradiction in the `states` CSS and the `WheelHolder::human` row. Matched the ATIS TAPE cell to `TapeConnection::open` (`since` time). Cited UX guideline 8 (#78) in the loading ban. Logged the holder-colour deviation from the exploration and how kind stays readable.
