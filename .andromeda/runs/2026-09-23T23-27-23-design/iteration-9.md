...
- `--rule-strong: 2px` (focus ring; the departure-blue left tick of the outbound and inbound transfer markers; the reserved v1.x Raised-3)
...
- **Anatomy:** `display: grid; grid-template-columns: var(--strip-cols); block-size: var(--strip-h);` on the holder surface #2A2E33, with a 1px `--rule-deco` outer edge and 1px `--rule-field` dividers between cells. At 760–1023px (Navigation → Width) the strip instead uses `grid-template-columns: var(--strip-cols-wrapped); grid-template-rows: var(--strip-rows-wrapped); grid-template-areas: var(--strip-areas-wrapped);` (52px block, band spanning both rows), and the `--rule-field` divider also runs between the two rows.
...
- **Content:**
  ...
  - `TAPE live` / `TAPE connecting` (lamp-off) / `TAPE stopped · viola ui not answering`. `TAPE stopped` is boxed with a 1px buff rule like `budget-paused` (the `TapeConnection::closed` border), because the stopped tape is a declared state.
...
### 4. Token Test
Every colour traces to one of the eight `--c-*` values (#1E2124, #2A2E33, #4A5057, #E6D8AE, #9C9278, #3B3A36, #D97706, #5B8DB8). `--c-*` is read only where a role alias is assigned or reassigned (`@layer tokens`, and the `stale` swap in `@layer states`). Components use the role aliases (`--surface-*`, `--ink*`, `--rule-*`, `--attention`, `--handoff`, `--focus-ring`, `--rb-*`) and never `--c-*`. Every length traces to a token (`--space-*`, `--fs-*`, `--track-*` (em), `--lh-display`, `--lh-label`, `--strip-h`, `--line-h`, `--band-w`, `--rb-size`, `--rule`, `--rule-strong`, `--radius`, `--cock-offset`, `--focus-w`, `--focus-offset`) or to a `ch` track in `--strip-cols` / `--strip-cols-wrapped` / `--tape-cols`, with the wrapped strip's rows in `--strip-rows-wrapped`. Every duration is `--cock-dur` or `--fade-dur`. A ninth hex value or an untokened px value signals no system.
...
[Iteration 5] [substantive] Added the transfer-marker tick to `--rule-strong`, removed the CLI "67 reserved" blue that contradicted the colour ban, and limited CLI amber to the `DIALOG` word. Defined the drawing of the `Window::expired` lamp-off box in the ATIS anatomy and Border Progression. Completed the Token Test (`--fs-*`, `--track-*`, `--radius`, wrapped strip tokens, durations). Made tape scroll and text-only updates explicitly instant. Named Motion → Hard limits as the canonical ban list and added the #81 / #31 command palette, toast and auto-refresh bans to web-spa.
[Iteration 6] [substantive] Gave the strip anatomy its 760–1023px wrapped template. Aligned the Typography Display usage with the ATIS anatomy. Completed the authoritative domain rows: the cocked + `stale` kind word is lamp-off, and `DialogPending` / `CliVersion` get unwrapped `n/a` rows. Defined the `TAPE stopped` buff box and added it to Border Progression and Core Colors. Brought Core Colors usage in line with Border Progression, including the `expired` box. Fixed the garbled inbound-arrow sentence in Iconography.
[Iteration 7] [substantive] Made the ATIS header wrap at cell boundaries (its nominal content is about 1090px against 992px available at 1024px) and gave each ATIS word a type role. Added `--lh-display` / `--lh-label` tokens and named them in the Token Test. Stated the real tape readback-word offset. Aligned the Signature Test and Brand Identity with component 3's no-box marker before the first send. Resolved the CLI cocked + `stale` row so `DIALOG` stays amber, matching the web.
[Iteration 8] [substantive] Fixed the Text Hierarchy Tertiary size claim to match the Typography table (13px Data/Body). Gave the Body role its 600-weight values (`human`, `DIALOG`) and added `BAY` to Label. Listed every rail use in Border Progression Subtle and stated that the #4A5057 ATIS-row border is the header rule, not a box. Logged the UX guideline 5 (#19) content-shift deviation. Specified the web ellipsis for over-long unwrapped names, with a scoped exception to the keyboard-reach ban. Stated that the CLI BAY line omits `VIOLA` and the address.
