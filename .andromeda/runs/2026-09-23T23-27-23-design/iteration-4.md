...
- **Transfer arrows.** On the web, `→` (U+2192) marks the outbound marker and `link` / `unlink` tape lines, and `←` (U+2190) marks only the inbound twin (`← overseer`). The CLI uses ASCII `->` and has no inbound form. `viola list` carries no link column in v1, so the six columns shared with the web strip stay fixed. Links print only as `link` / `unlink` verb output (`overseer -> builder`).
...
- stderr: context lines (`waiting: <name>`, the issue line of a send), refusals (`unable  <reason>  <detail>`, two-space separated as in the exit-code table; only the exit-1 start refusal uses the fixed message form `unable: <name> is already live`) and errors (`error: …`), and the `viola ui` launch line.
