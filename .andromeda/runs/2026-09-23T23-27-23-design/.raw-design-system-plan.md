...
- Fallbacks stay in the same families: DIN Alternate on macOS, DejaVu Condensed and DejaVu Mono on Linux. They end at the generic `sans-serif` / `monospace`, never `system-ui`.
...
**Loading:** System font stack only, with named installed faces referenced by `font-family`. Nothing is fetched, which keeps the page CSP-clean.
- `--font-label: "Bahnschrift", "DIN Alternate", "Avenir Next Condensed", "DejaVu Sans Condensed", sans-serif;`
- `--font-field: "Cascadia Mono", Consolas, Menlo, "DejaVu Sans Mono", "Noto Sans Mono", monospace;`
- Semi-condensed labels use `font-stretch: 87.5%`, which Bahnschrift's variable width axis honours. On the fallbacks it is a no-op.
...
- **Fonts:**
  - Bahnschrift ships on Windows 10 1709+. Cascadia Mono ships with Windows Terminal on Windows 11; on Windows 10 without Terminal the stack falls to Consolas.
  - The headless check must assert the computed family is not a banned face on the CI images. DejaVu is the expected Linux face.
