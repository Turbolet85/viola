...
This number is the SINGLE SOURCE OF TRUTH for how much motion and visual dynamism downstream phases implement. A 0.2 project gets `transition: none` or `150ms ease-out` at most. viola's web page is 0.3, but its only two motions are the ones in the table above.
...
| Accent | #D97706 | The only attention colour: the 4px holder band and the `DIALOG` word of a cocked strip, the page-title marker, and the CLI `DIALOG` column. Never used for errors, warnings, budget or hover. | Arrival-holder amber: the orange holder cocked out of line. It is the physical "needs you". |
| Role (handoff) | #5B8DB8 | Transfer-marker arrow, link names and link tape lines only. Never a fill and never a status. | Departure-holder blue: outbound traffic being handed to the next sector (`driver → driven`). |
...
| Info (activity, harness turn, skipped counts, tape status) | #1E2124 | #4A5057 | #9C9278 |
...
| Label | Bahnschrift stack, semi-condensed | 400 | 11px / 16px, uppercase | 0.06em | Column captions (NAME · LIVE · STATUS · WHEEL · DIALOG · CLI), `5H` / `7D`, `TAPE`, and `DIALOG` (weight 600 there) |
...
| Token | Value | Usage |
|-------|-------|-------|
| space-micro | 4px | Gap between the readback box and its word cell, between `→` and a name, and the inline padding inside a field cell |
| space-xs | 8px | Field-cell inline padding in strips, rack gap between strips, the transfer-marker indent step |
| space-sm | 12px | Padding inside the ATIS header cells. Equals `--cock-offset`: the rack reserves this much end padding so a cocked strip never causes horizontal scroll. |
...
**High-impact moments** (max 1 at base; the web surface's 0.3 allows the named pair and nothing more):
...
- **Transfer arrow.** `→` (U+2192) on the web. The CLI uses ASCII `->`.
...
@layer tokens {
  :root {
    ...
    --tape-cols: 12ch minmax(8ch, 16ch) 17ch minmax(0, 1fr) var(--rb-size) minmax(8ch, 36ch);
...
@layer states {
  ...
  viola-session-row[data-live="stale"] { --surface-strip: var(--c-anthracite);
...
- **Column captions** appear once per rack, in a header row on anthracite: Bahnschrift 11px uppercase, lamp-off, `role="columnheader"`. They are never repeated inside strips, because lamp-off on holder fails AA.
- **States:**
  - Default (live, lit).
  - `stale`: an instant lamp-off swap (see the `states` layer).
  - Cocked: `data-dialog="pending"`.
  - Cocked + `stale`: the cock wins for offset, band and `DIALOG`, and the other cells stay lamp-off.
  - Hover: none.
  - Active: n/a.
...
- **Anatomy:** a 20px line in the rack gap on anthracite, directly under the driver's strip. It is indented by `--band-w + --space-xs` and has a 2px departure-blue left tick. There is one marker per link, ordered by driven name.
- **Content:**
...
- **Inbound twin:** the driven strip gets `← overseer` (blue) with no box.
...
- **Anatomy:** a `role="log"` panel on anthracite. Each line is a `<details>` with `display: grid; grid-template-columns: var(--tape-cols)` and a 20px row.
  ...
  - The RB box and word or reason, on send lines only. Other lines let TEXT span to the end.
...
Otherwise: 256-colour, or truecolor if `COLORTERM` says so.

**Streams:**
- stdout: results and data (tables, send/wait results, `last` text).
- stderr: context lines (`waiting: <name>`, the issue line of a send), refusals (`unable: …`) and errors (`error: …`), and the `viola ui` launch line.
...
| 21 | `unable  instance-unreachable  <name>` | `{"v":1,"error":"instance-unreachable","detail":null}` |
...
### 3. Signature Test
Point to the signature element in your output. Can you find it in at least
3 places? If you can't locate it → it doesn't exist. Inject it.

### 4. Token Test
Read your color/spacing values aloud. Do they trace back to the palette
and scale above? Random hex values or magic numbers signal no system.
