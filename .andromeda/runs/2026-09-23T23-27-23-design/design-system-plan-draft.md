...
[Iteration 10] [substantive] Defined a CLI "glyph" as non-ASCII, so the `[RB]` / `[  ]` / `[/ ]` mirror prints in every human output. Scoped lamp-off `since` times to transfer markers, since the ATIS `TAPE live since` time is buff. Separated `--rule-strong` from the focus-ring tokens. Placed `hint:` lines on stderr, not printed under `--json`, in both Streams and the stream ban. Moved the exit-0 result line to stdout in the exit-code table. Aligned the "done or success" ban example with the padded `viola send` sample.
