...
The last three issues (the `space-md` gap, the exit-1 hint, the Core Colors anchor rows) and the Rejected Defaults format note have no patch. The first two need an owner's choice rather than a correction. The last two are only presentation: the content is already there.

File checked: `D:/dev/projects/viola/.andromeda/runs/2026-09-23T23-27-23-design/design-system-plan-draft.md`, against `exploration.md` and `library-shortlist.md` in the same folder.
