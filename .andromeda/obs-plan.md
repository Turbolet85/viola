...
6. **Verification:** the tests secret-scan test (token, `Cookie`, `?t=`, `CLAUDE*` values, 0600) over every `diagnostics/` file, run in CI before artifact upload.
   ...
   - The same rule covers the `harness-<os>` artifact. `agent-run logs` merges `events.ndjson` records (`prompt-submitted.data.text`, `last_assistant_message`) and detail lines, so it carries the canary by design. The scan applies the Critical-class patterns to `target/agent-run/*` except exactly `target/agent-run/chunk.diff`, and the canary rule exempts those files as it exempts `detail-*.ndjson`. That one file is the mutation leg's diff, repository source text by construction (it hits `?t=` whenever it touches the scan's own patterns); the `harness-<os>` upload excludes it (`!target/agent-run/chunk.diff`), so it is never scanned and never uploaded. A `chunk.diff` anywhere else under the capture is still scanned.
   ...
   - **Unscanned uploads, admissible by content** (operator ratification 2026-09-24):
     ...
     - the ci.yml `supply-chain` artifact (`target/supply-chain/deny.json`, `deny-fuzz.json` for the `fuzz/Cargo.lock` audit, and `zizmor.json`; `if: always()`, 7 days) holds cargo-deny and zizmor diagnostics only: crate names and versions, advisory ids, registry URLs and workflow-relative locations. It carries no absolute path, argv, log content or user input (as measured at chunk 2026-09-24-workspace-tree-and-code-graph-planes: 0 absolute paths in all three files; the one raw pattern hit was the `s:/` inside `https://`). The guard stays binding: a member that could carry a host path or log content is scanned before upload, or dropped (operator ratification 2026-09-24, logged for founder review).
   - **Scan failure:**
...
| Mutation | obs code (`obs_event!` call sites, `MillisUtc`, panic hook, TraceLayer closures) under cargo-mutants like product code | per-leg `mutants-verdict-<os>.json` (ubuntu-latest, windows-2025), merged by the `mutants-verdict` job's `gate --require mutants --mutants-legs …`: each mutant is judged only by the legs whose `#[cfg]`s compile its line (every leg when none does) and is red when no judged leg caught it and some judged leg missed it or timed out, so a missed obs-code mutant on its compiling leg stays red; before that union, a leg whose unviable mutants outnumber its caught ones is red at its own run (test-plan §10 Mutation gate) |
...
3. Secret scan, `id: secret-scan`, `if: always()`, over `target/e2e-home/**/diagnostics/*.ndjson` and `target/agent-run/*` except the mutation leg's `target/agent-run/chunk.diff` (skipped by exact path and excluded from the step-4 `harness-<os>` upload), and also `target/nextest/ci/junit.xml`, because nextest stores failing tests' captured output in it. Without `always()`, a failed E2E step skips the scan (outcome `skipped`), and the gated uploads are then skipped too, which loses the failing job's diagnostics (§11 CI).
