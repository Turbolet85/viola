...
- **[Error Handling] thiserror 2.0.20, one enum per workspace crate; anyhow 1.0.104 only in the `viola` bin: its `main`, dispatch (`cmd::dispatch` returns the error with the resolved home and instance) and the catch-site reporter `viola::obs::report_internal_error`.** `RefusalReason` lives in `viola-core`. Rationale: callers branch on refusal variants across three surfaces (channel, CLI exit codes, MCP `isError`). anyhow's context chains are only useful where a person reads them: the owner's post-mortem, which gets the chain as `chain` in `instances/<name>/diagnostics/detail-<role>.ndjson`, never a role line, stdout or stderr.
...
- `config.json`: user settings (budget thresholds, GUI port, `diagnostics_level`: `"info"` by default, or `"debug"`). It is read with a `MAX_FRAME` cap and must carry `v` = 1. An unusable file or value falls back to the default and is reported as a `parse-rejected{parser:"config-json"}` line.
...
- `target/orphans-probes/run-<utc>-<pid>/`: the two throwaway crates `scripts/orphans-check.sh --probe` writes per run (a planted `src/stray.rs` and a control, each with an empty `[workspace]`), gitignored.
...
- Code-graph planes (`scripts/code-graph.py`, one DuckDB per language plane under `.andromeda/cache/{plane}/`):
  - **rust:** the root workspace through rust-analyzer SCIP. It indexes every member (`viola`, `viola-core`, `viola-e2e`). `fuzz/`, a separate workspace, is outside it.
...
- Licence: the project is `MIT OR Apache-2.0` (founder ruling 2026-09-25). `license` is set once in root `[workspace.package]` and inherited with `license.workspace = true` by `viola`, `viola-core` and `viola-e2e`; `fuzz/Cargo.toml`, its own workspace, carries the literal. The texts are `LICENSE-MIT` (holder Turbolet85) and `LICENSE-APACHE` at the root, and `README.md` names both.
...
- serde, serde_json, chrono and thiserror are shared by every workspace crate through `[workspace.dependencies]`. The sync crates (`viola-core`, `viola-state`, `viola-agent-claude`) list chrono directly, because they do not reach it through rmcp.
...
- **Config management.** Precedence: CLI flags override `<viola home>/config.json`, which overrides built-in defaults.
  ...
  - Settings: budget thresholds as `Percent` (defaults `five_hour` 90, `seven_day` 85), the GUI port (default 47319) and `diagnostics_level` (`info` by default, or `debug`). `diagnostics_level` is the only source of the process-log level; `RUST_LOG` has no effect.
