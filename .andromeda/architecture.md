...
- **No daemon; the disk is the shared truth.** Every `viola run` owns its own local-socket endpoint, every other verb is a separate process that owns no endpoint (short-lived, except the long-running `mcp` and `ui`), and the state all processes share lives in ndjson logs and atomic snapshots that survive a crash on either side.
...
|---|---|---|
| Language / runtime | Rust 1.98.1, pinned exactly by `rust-toolchain.toml` (rustfmt + clippy); workspace `rust-version = "1.96"`, edition 2024 | One native binary `viola` / `viola.exe`. 1.96 is the declared floor (the security toolchain floor; sysinfo 0.39.6 needs 1.95, std `File::lock` 1.89, rmcp 1.88, axum 1.80). CI checks the floor in the `msrv` job: 1.96 is installed by `rustup toolchain install 1.96` and selected with `RUSTUP_TOOLCHAIN=1.96`, which outranks `rust-toolchain.toml`, so the pin is untouched. The separate `fuzz/` workspace alone builds on `nightly-2026-09-20`, pinned by `fuzz/rust-toolchain.toml` |
...
| Wrapper IPC | interprocess 2.4.4 `local_socket` (sync API; Tokio flavour only in `viola-mcp`); on Windows the client opens the pipe itself with windows-sys 0.61.2 `CreateFileW(SECURITY_SQOS_PRESENT \| SECURITY_IDENTIFICATION \| FILE_FLAG_OVERLAPPED)` and adopts the handle with `local_socket::Stream::try_from`, never interprocess's default connect | One endpoint per `viola run`: a named pipe on Windows, a Unix domain socket elsewhere. The Windows open leaves the server able to identify, never impersonate, the client (as measured at chunk 2026-09-25-security-prerequisites; security-plan Decisions Log `2026-09-25`) |
...
| ORM / migrations | N/A. Every record carries `v`, readers skip unknown kinds and fields, and a snapshot with an unsupported `v` is rebuilt by replaying the log. The exception is process-log lines (home-level role files and instance detail files): they carry no `v`, and their version lives in the schema filename (`schemas/diag-line.v1.json`, `schemas/diag-detail.v1.json`), because obs's default-deny field list admits no `v` key | Stands in for schema migration |
...
| Content hash | sha2 `=0.11.0` (`default-features = false`; pure Rust, no `cc`) | SHA-256 of the exe bytes, truncated to the first 8 bytes (16 hex), for the `bin/` and `plugin/` `<version>-<hash>` keys. Pinned today as a root dev-dependency by its known-answer test; `viola-state` is its first product consumer |
...
| CI/CD | GitHub Actions matrix `windows-2025`, `macos-latest` (macOS 26), `ubuntu-latest`; toolchains installed by `rustup toolchain install`: from `rust-toolchain.toml`, from `fuzz/rust-toolchain.toml` in the fuzz jobs, and `1.96` in the `msrv` job (no toolchain action); SHA-pinned actions/checkout 7.0.1, Swatinem/rust-cache 2.9.2, taiki-e/install-action 2.87.19, actions/upload-artifact 7.0.1, actions/download-artifact 8.0.1 | Build, lint and test on all three OSes against the fake agent, on native runners |
...
- **[Concurrency / Backend Framework] Hybrid sync core + Tokio only in `mcp` and `ui` (KEYSTONE).** Rationale: portable-pty's handles are blocking, so the pump needs threads either way. `hook` spawns once per hook event and must not pay for runtime startup. rmcp, axum and interprocess-async all require Tokio, so it is confined to the subcommands that use them. CI enforces this with a sole-root `cargo deny` `tokio` ban (`deny-sync.toml`, run once per crate listed in `scripts/sync-crates.txt`), evaluated for the Windows, macOS and Linux target triples, plus a `cargo check` of the listed sync crates without the `tokio` feature on all three OSes (CI job 3).
- **[PTY] portable-pty `=0.8.1` pinned behind viola's own `pty` seam (spawn · read · write · resize · wait · kill).** Child exit is detected on the process handle, never on EOF, because ConPTY does not close the output stream on exit. If 0.8.1's Windows `kill()` misbehaves, the fallback is `TerminateProcess` on `child.process_id()` via windows-sys. Rationale: 0.8.1 is spike-proven (S1, O1 render stress, the Pulse live test, and about an hour driving this arch session in Windows Terminal with no PTY defect). 0.9.0 is excluded because of the Windows garbage-read bug wezterm#6783. Swap triggers: portable-pty-psmux 0.9.7 if the O1 stray-spaces artifact reproduces only under `viola run` (then test `PASSTHROUGH_MODE`). An own ConPTY on windows-sys 0.61.2 + nix 0.31.3 comes only after a measured defect the seam cannot absorb. As built (chunk 2026-09-25-pty-wrapper-on-windows, measured on the Windows host in its research facts 1-5): `viola run` puts its own host terminal into raw input + VT output for the run through `HostTerminal` (Windows console modes; Unix termios) and restores the saved modes on Drop — without it ConPTY line-buffers keys until Enter and swallows Ctrl-C; off a terminal it is a no-op. The child's cwd is set explicitly to viola's current directory (portable-pty 0.8.1 defaults to `%USERPROFILE%`). viola resolves the child program to an absolute path itself (portable-pty's Windows search tries the exact name first and hits the extensionless npm sh shim). The pump keeps the ConPTY input writer until the child exits, because dropping it makes conhost send `CTRL_CLOSE` to the child (exit `0xC000013A`). The exit is recorded as `exit_source` `handle-wait` or `kill-fallback`.
...
- **[Message Broker / IPC] interprocess 2.4.4 `local_socket`, one endpoint per `viola run`, behind viola's own `channel` seam.** `hook` blocks on the channel for a dialog answer. `wait` blocks until the next driver-relevant event (`turn-ended`, `question`, `permission`, `plan` or `session-end`) and returns that event, so nothing polls and a driver parked in `wait` learns that a dialog is pending. If a dialog is already pending when `wait` is called without `after`, or with an `after` at or before that dialog's event line, it returns that dialog at once. With an `after` past that line the driver has already seen the dialog, so `wait` blocks for the next driver-relevant event, and a driver that leaves a dialog to the human never spins. Each dialog event carries a wrapper-assigned `dialog_id` (an integer, monotonic per instance, with the counter restored from `events.ndjson` on start so an id never repeats across wrapper restarts), and `answer` names its target by that id. At most one dialog is pending per instance. A `hook.dialog` that arrives while another is pending is answered `null` at once, so that dialog renders for the human. Its event is still logged with its own `dialog_id`, and a later `answer` for it is refused `not-delivered` with detail `unknown-dialog`. Whether Claude Code can hold two dialog hooks open at once (for example on parallel tool calls) is a capability-ledger row. `wait` takes an optional `after` cursor and an optional `timeout_ms`. `after` is a byte offset into the instance's `events.ndjson`. `wait` returns the first driver-relevant event whose line starts at or after it, at once if that event is already logged, so a turn that ends before the driver calls `wait` is never missed. Every `send` `ok` payload and every `wait` event result carries `cursor`, the offset to pass as the next `after`. Without `after`, `wait` returns the next such event appended after the call. With no `timeout_ms` it blocks until an event arrives, and if the endpoint vanishes first the caller gets `instance-unreachable`. On timeout it returns `ok` with `{"timed_out":true}`. `last` returns the `last_assistant_message` of the newest `turn-ended` event. The wrapper holds it in memory and rebuilds it from `events.ndjson` on start. Endpoint names are a short hash of the instance, never the raw `VIOLA_NAME` + path. On Windows the client connects only through the SQOS Identification open (windows-sys `CreateFileW` with `SECURITY_SQOS_PRESENT | SECURITY_IDENTIFICATION | FILE_FLAG_OVERLAPPED`, adopted by `Stream::try_from`), never interprocess's default connect, so a squatted or rogue server cannot impersonate the client; a non-overlapped handle adopted this way hangs (as measured at chunk 2026-09-25-security-prerequisites). Rationale: one API over named pipes and Unix sockets, sync and Tokio flavours, and no daemon. macOS caps socket paths at about 104 bytes. Accepted risk: interprocess has one main maintainer, which the seam contains.
...
- **[MCP] rmcp 3.4.1, stdio, tools `send · wait · last · answer · list`. `release` is CLI-only.** Refusals are `isError: true` tool results mapped from `result.refusal`. Rationale: it is the official SDK, tracks spec 2026-07-28, and the pinned minor limits churn from its near-weekly releases. `answer` and `list` are how a driver answers dialogs under R7 and sees sessions, and the prototype driver uses both every session. `release` stays off the driver's tool surface, so a driver is never offered a way to hand the wheel back to automation. This is an affordance, not an enforcement: an agent's Bash tool can still run CLI `viola release` (for example through `VIOLA_BIN`), and the Local endpoint trust boundary accepts it because v1 trusts every process of the same OS user. Whether a wrapper should refuse a `release` whose `params` carry `from` is left to the security specialist. `pause`, `link` and `unlink` are CLI-only for the same reason: they are human controls over the wheel and the link topology, and links made by drivers are already recorded from `from` on `send` and `answer`. Tool inputs:
  - `send {target, text}`
...
- **[Hook Transport] Exec-form `command` hooks: `"command": "<pinned viola copy>", "args": ["hook", "<event>"]`.** Tiers:
  ...
  - sync with no reply body (the ordering spine): SessionStart, UserPromptSubmit, Stop. Each writes its `hook.event` and exits 0 within viola's spine deadline, an open item: one built-in value, below the `timeout` those hooks get in `hooks.json`. If the endpoint cannot be reached or written within the deadline, the hook records the failure in `diagnostics/` and exits 0. The event is lost rather than the turn delayed, and a driver recovers through `wait`'s `timeout_ms`.
...
- **[CLI Version Compatibility] Per-behaviour capability ledger + transport-only degrade + `viola verify` re-stamp.** Ledger rows:
  ...
  - The R8 identity floor: the measured `CLAUDE*` parent-identity names stripped even when the persistent environment defines them (every other `CLAUDE*` name follows the prefix rule, see Occupied Resources → Environment variables).
  ...
  - The hook tier map.
  - Turn end without Stop: which hook, if any, fires when a turn ends on an API error (Stop does not fire on a user interrupt), so the running-turn state can be cleared.
  - Dialog concurrency: whether two dialog hooks (PreToolUse `AskUserQuestion|ExitPlanMode`, PermissionRequest) can be open at once in one session, for example on parallel tool calls.
  - `claude agents --json` join field: whether a row exposes the child pid or the session id that matches a wrapped instance.
  - Plugin precedence: whether a `--plugin-dir` plugin and an installed plugin with the same name both load, and which MCP server `viola` wins.
  ...
  - Tag escaping: the CLI inserts a backslash after `<` in tag-like text the user typed, so a literal `<pasted_content` arrives in UserPromptSubmit as `<\pasted_content`. `viola-agent-claude` reverses this escaping, after removing any long-paste wrapper, when it normalises `prompt-submitted`, so delivery matching compares plain text.
...
- **[Session Liveness] Heartbeat file primary; pid + process start time and `claude agents --json` only enrich it.** Rationale: the prototype's heartbeat proved reliable. A bare pid is unsafe because pids get reused, which is why start time is matched too. `claude agents --json` adds idle/busy status and shows unwrapped sessions as read-only rows. A `viola run` whose `ViolaName` has an answering endpoint or a `stale` heartbeat refuses to start (exit 1, with a message naming the live instance). Otherwise `run` starts in this order: collision check, pinned copy and plugin folder, version gate, exclusive bind of the endpoint (a bind that finds the name taken is the same exit-1 refusal, so two concurrent starts cannot both win), first snapshot (with `endpoint`, `pinned_bin` and `statusline_command`) and heartbeat, the start `wheel` and `budget-gate` events, and only then the child spawn. The child's first SessionStart hook therefore always finds a listening endpoint and a recorded snapshot. A gone instance's directory is reused, and its `events.ndjson` is appended to, never truncated.
...
- **[Error Handling] thiserror 2.0.20, one enum per workspace crate (one ratified exception: `viola-pty`'s `PtyError` has a hand-written `Display`/`Error` with fixed messages, because the seam's normal dependencies are exactly portable-pty, windows-sys and libc; portable-pty's anyhow error is kept only as a boxed source); anyhow 1.0.104 only in the `viola` bin: its `main`, dispatch (`cmd::dispatch` returns the error with the resolved home and instance) and the catch-site reporter `viola::obs::report_internal_error`.** `RefusalReason` lives in `viola-core`. Rationale: callers branch on refusal variants across three surfaces (channel, CLI exit codes, MCP `isError`). anyhow's context chains are only useful where a person reads them: the owner's post-mortem, which gets the chain as `chain` in `instances/<name>/diagnostics/detail-<role>.ndjson`, never a role line, stdout or stderr.
- **[Module Boundaries] Cargo workspace, flat `crates/` layout, with the root `Cargo.toml` as both the `viola` bin package and `[workspace]`.** Members: `viola-core`, `viola-pty`, `viola-channel`, `viola-state`, `viola-agent-claude`, `viola-mcp`, `viola-ui`, plus the test-only `viola-e2e` (the `viola-harness` agent-run harness; no product crate depends on it). Each product crate is created by the first chunk that consumes it. `fuzz/` (the `viola-fuzz` cargo-fuzz targets) is NOT a member. It is its own cargo workspace, excluded by the root's `exclude = ["fuzz"]`, so its nightly toolchain and C++-building libfuzzer-sys never enter the root graph. Only `viola-mcp` and `viola-ui` list `tokio`, and `viola-channel`'s Tokio client sits behind a `tokio` feature only `viola-mcp` enables. Rationale: the compiler enforces that `viola-pty` cannot see `viola-agent-claude`. `cargo install --path .` requires the bin at the root. cargo_pup needs nightly and was rejected.
...
- **[CI/CD] GitHub Actions native matrix on three OSes, tests against the fake agent, the real CLI only in local live tests.** The fake agent is substituted by naming its executable after `--` (`viola run <name> -- <fake agent> <args>`). viola resolves the child program to an absolute path itself (on Windows a PATH search over PATHEXT only, never the bare name) and spawns that. A `.cmd`/`.bat` child is refused (exit 1, `batch-script-child`), except the `claude` npm shim: `claude.cmd`/`claude.bat` resolves to `<dir>/node_modules/@anthropic-ai/claude-code/bin/claude.exe` when that file exists, and the shim is never read. Each test runs in its own viola home (`--home`, see Config management), so parallel tests never share an endpoint, log or `budget.json`. Rationale: no tokens spent and no flakes in CI, and no cross-compilers (cross 0.2.5 is stale).
...
- **`RefusalReason` (serialised kebab-case):** `human-typing` · `budget-paused` · `unverified-cli` · `not-delivered` · `unknown`. `unknown` is `#[serde(other)]`, so an older client can still read a refusal from a newer wrapper. `detail` values are kebab-case strings from a closed set per reason, extended only in `viola-core`:
  ...
  Readers treat an unrecognised `detail` as opaque text, never as an error. When several refusals apply, the wrapper returns only the first in this order. For `send`: `human-typing`, `budget-paused`, `not-delivered`/`turn-running`, then the readiness gate (`input-not-ready`) and confirmation (`no-prompt-submitted`) outcomes. For `answer`: `human-typing`, `unverified-cli`, `not-delivered`/`unknown-dialog`. `unconfirmable` is not a refusal detail. It appears only in an `ok` payload (`{"confirmed":false,"detail":"unconfirmable"}`), for a ledger-listed local command whose post-condition is "none" or not yet measured on this CLI version.
- **CLI exit codes:**
  - `0` ok
  - `1` internal error (anyhow edge), `viola run` refusing to start because its `ViolaName` has an answering endpoint or a `stale` heartbeat ([Session Liveness]), and `viola run` refusing a `.cmd`/`.bat` child program before any spawn (`process-exit{subject:"self", exit_code:1, detail:"batch-script-child"}`, the two fixed `unable:`/`hint:` stderr lines)
...
- **Rust error types:** one error enum per crate, named `<Crate>Error` (`PtyError`, `ChannelError`, `StateError`, `AgentError`, `McpError`, `UiError`, `CoreError`), each a `thiserror` enum except `PtyError`, whose `Display`/`Error` are hand-written with fixed messages ([Error Handling]). nutype validation errors fold into the owning crate's enum. `anyhow` appears only in the root bin crate.
...
- **Normalised event kinds (`viola-core`):** `session-start`, `turn-ended`, `prompt-submitted`, `question`, `permission`, `plan`, `session-end`, `activity`, `link`, `unlink`, `wheel`, `budget-gate`. New kinds are added only in `viola-core`, never with Claude-specific names.
...
- `/api/sessions` item: `{name, wrapped, liveness, status, wheel?, budget_paused?, dialog_pending, cli_version?, cli_verified?}`.
  ...
  - `wrapped: false` marks a read-only unwrapped row from `claude agents --json`. Its `name` is the session identifier that `claude agents --json` reports (part of the join-field ledger row). It is not a `ViolaName` and is never a valid `target`. Its `liveness` is `live` and `dialog_pending` is `false`, and `wheel`, `budget_paused`, `cli_version` and `cli_verified` are absent.
  ...
  - The `/api/sessions` envelope also carries a top-level `budget`: `{five_hour, seven_day, read_at, paused}`, or `"unknown"`. `five_hour` and `seven_day` are each `{used_percentage, resets_at}`, with either field `"unknown"` when unparseable and the whole window `"unknown"` when the statusline omits it, because the two windows reset independently and a budget pause lifts at the crossed window's own `resets_at`. `budget.json` holds the same per-window reading plus `read_at`. The page derives the reading's age from `read_at`.
...
```
A snapshot whose `v` is higher than the reader supports, or that fails to parse, is ignored, and the state is rebuilt by replaying `events.ndjson`. Replay recovers only log-derived fields: `links`, `agent_session_id`, `wheel`, `budget_paused` and `budget_override_until`. Fields no event carries (`endpoint`, `pid`, `started_at`, `pinned_bin`, `statusline_command`, `cli_version`, `cli_verified`, `child_pid`, `pending_dialog`) are treated as absent. Liveness then rests on the heartbeat alone, so the instance is `live` or gone and never `stale`. The endpoint is rebuilt as described under IPC endpoints, and `hook statusline` prints nothing.

**Instance snapshot** (`instances/<ViolaName>/snapshot.json`, written only by that instance's wrapper, rewritten on every change to a field below): `data` is `{endpoint, pid, started_at, pinned_bin, statusline_command?, cli_version?, cli_verified, wheel, budget_paused, budget_override_until?, pending_dialog?, links, child_pid, agent_session_id?}`.
- `pid` and `started_at` identify the wrapper process for the liveness check.
- `child_pid` is the pid of the spawned agent child, and `agent_session_id` is the one from the newest `session-start` event. Both exist only to join `claude agents --json` rows to this instance (see Session liveness).
- `cli_version` and `cli_verified` come from `run`'s version gate.
- `pending_dialog` is `{dialog_id, kind}` while a `hook.dialog` is open. It is removed when the dialog is answered, expires or is answered `null` on a wheel move. It is the only source of `dialog_pending` in `list` and `/api/sessions`. It is not rebuilt from the log, so a reader that falls back to replay reports `dialog_pending: false`.
- `links` is the set derived by replaying `link`/`unlink` events, as `{driver, driven, since}`.
...
**Event `data` per kind** (normalised in `viola-agent-claude`; upstream text is content, never commands, per R1):
- `session-start`: `{cause, agent_session_id}` (see the Hook → kind map).
- `prompt-submitted`: `{text, origin}`. `text` is the submitted prompt as it was sent: `viola-agent-claude` removes any `<pasted_content id=…>` long-paste wrapper and then reverses tag escaping before emitting the event, and delivery matching compares it with the sent text directly. `origin` is `driver` · `human` · `harness`.
- `turn-ended`: `{last_assistant_message}` (string or `null`). This is the source `last` rebuilds from.
- `question`: `{dialog_id, questions:[{question, options:[<label>], multi_select}]}`. Each `question` string is a key in `answer`'s `answers`.
- `permission`: `{dialog_id, tool, input}`. `input` is the tool call's arguments as a JSON value, carried as content, not the hook payload.
- `plan`: `{dialog_id, plan}`, with the plan as text.
- `session-end`: `{}`.
- `activity`: `{}`, plus `tool` when the hook names one.
- `link` / `unlink`: `{driver, driven}`.
- `wheel`: `{holder, cause}`, with `holder` `human` · `driver` and `cause` `start` · `human-input` · `manual-pause` · `release`.
- `budget-gate`: `{paused, window?, override_until?}` for the instance's own gate, with `window` `five-hour` · `seven-day`.

The wrapper appends `wheel` and `budget-gate` (`source: wrapper`) once at start and on every change. They are log-only and never wake `wait`. They are how the audit trail, the SSE feed and a replay learn the wheel holder and the budget gate.

`hook.dialog`'s `data` is the same object as its dialog event's `data`, without `dialog_id`. Dialog hooks send only `hook.dialog`, never a `hook.event` for the same dialog. On receipt, the wrapper assigns the `dialog_id` and appends the dialog event (`source: hook`) before it can wake `wait` or reply. It does this even when it replies `null` at once (wheel `human`, unverified CLI, or another dialog already pending), so every dialog appears exactly once in the log.
...
- `claude agents --json` adds `status` (idle/busy) and lists unwrapped sessions as read-only rows. Its readers (`list`, `mcp` and `ui`, through `viola-agent-claude`) spawn it directly, resolving `claude` from their own PATH with the same npm-shim → `claude.exe` resolution `run` uses. If it is not found, fails or cannot be parsed, every row's `status` is `unknown`, no unwrapped rows are listed, and the outcome is never an error. A row that matches a wrapped instance's `child_pid` or `agent_session_id` in its snapshot is merged into that instance's row, never listed separately. Which of the two fields `claude agents --json` exposes for this join is a capability-ledger row (open item).
...
- **Test-only (Windows):** named pipe `\\.\pipe\viola-test-sqos-<pid>-<label>`, created by `tests/channel_sqos_open.rs`. It is outside the `viola-<h12>` namespace (the `test-` segment is never 12 lowercase hex characters), unique per test and never bound by `viola`.
- **Linux / macOS:** Unix domain socket file `$TMPDIR/viola-<h12>.sock` (`/tmp` when `TMPDIR` is unset), which stays within the ~104-byte macOS limit. `run` resolves this path once and records it as `endpoint` in the instance snapshot. Every other process (`hook`, `mcp`, CLI verbs, and `run`'s collision check) connects to the recorded `endpoint`, because processes spawned inside the session may run with a different `TMPDIR` from the wrapper. It rebuilds the path from its own `TMPDIR` only when the snapshot is missing or unreadable. Linux abstract-namespace names are never used, because they carry no filesystem permissions and any local user could connect (see Local endpoint trust boundary).
...
- Per-session settings override: `instances/<ViolaName>/settings.json`, written by `viola run` at start (it wraps the statusline with `viola hook statusline`) and passed to the child through the settings-override mechanism recorded in the capability ledger.
...
- Landed so far: `viola`, `viola-core`, `viola-pty`, `viola-agent-claude`, `viola-e2e`. `viola-pty` and `viola-agent-claude` are sync crates (listed in `scripts/sync-crates.txt`) and, like `viola-e2e`, each declares a code-free `fake-agent = []` feature so a package-scoped `--features fake-agent` build (cargo-mutants) resolves.
- Not a member: `viola-fuzz` (`fuzz/`, test-only, `publish = false`). It is its own cargo workspace with its own `Cargo.lock`, excluded from the root, and depends on `viola-core` by path.
...
- Removed from the child's environment (R8, operator ruling 2026-09-25): every inherited `CLAUDE*` name (ASCII case-insensitive on Windows, exact on Unix) except a name the user defines persistently — on Windows a value NAME under `HKCU\Environment` or `HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment` (read through windows-sys Registry, names only), on Unix a name in `config.json` `claude_env_keep`. The identity floor (`viola-agent-claude` `IDENTITY_FLOOR`, a ledger row; the 11 names measured on the Windows host: `CLAUDECODE`, `CLAUDE_CODE_BRIDGE_SESSION_ID`, `CLAUDE_CODE_CHILD_SESSION`, `CLAUDE_CODE_ENTRYPOINT`, `CLAUDE_CODE_EXECPATH`, `CLAUDE_CODE_MESSAGING_SOCKET`, `CLAUDE_CODE_MESSAGING_TOKEN`, `CLAUDE_CODE_SESSION_ATTENDED`, `CLAUDE_CODE_SESSION_ID`, `CLAUDE_EFFORT`, `CLAUDE_PID`) is removed whatever the persistent environment says. Names only: the strip, the registry reader and the logs never read a value; `process-start{subject:"claude-child"}` names the stripped (`env_stripped_known`) and the kept (`env_kept`) names.
...
- Test-harness only, never read by `viola`:
  - `AGENT_RUN_CHUNK_BASE`: an explicit override of the mutation gate's diff base, read by `viola-harness` (`run_cmd`). Without it the harness derives the base from git history (the last master flip before the chunk's oldest operator pre-CI commit; test-plan §3 `run` step 4). CI does not set it.
...
- `config.json`: user settings (budget thresholds, GUI port, `diagnostics_level`: `"info"` by default, or `"debug"`; `claude_env_keep`: the Unix R8 pass-list, an array of at most 32 names each matching `^CLAUDE[A-Z0-9_]*$`, default empty — a bad value is rejected whole, and an identity-floor name in it has no effect). It is read with a `MAX_FRAME` cap and must carry `v` = 1. An unusable file or value falls back to the default and is reported as a `parse-rejected{parser:"config-json"}` line.
...
- `diagnostics/`: home-level per-role process logs, codes only: `run-<name>.ndjson`, `hook-<name>.ndjson`, `mcp.ndjson`, `ui-<port>.ndjson` and `cli-<name>.ndjson`. Only `run` has a producer today. One line per `write`; dir 0700, files 0600 on Unix; line format owned by obs and fixed by `schemas/diag-line.v1.json`. Content-bearing detail stays in `instances/<ViolaName>/diagnostics/`.
- `instances/<ViolaName>/events.ndjson`, `events.ndjson.lock`, `snapshot.json`, `snapshot.json.lock`, `heartbeat`, `settings.json` (the per-session settings override that wraps the statusline), `diagnostics/` (the owner-only content-bearing detail files `detail-<role>.ndjson`, such as the panic payload and backtrace in `detail-run.ndjson`; dir 0700, files 0600 on Unix; one `write` per line; file format owned by obs and fixed by `schemas/diag-detail.v1.json`. Codes-only process lines go to the home-level role files). v1 never rotates or truncates `events.ndjson`, because `wait`'s `after`, `send`'s `cursor` and SSE ids are byte offsets into it. Bounding its size is an open item, and any scheme must keep existing offsets valid.
...
- `schemas/diag-line.v1.json` and `schemas/diag-detail.v1.json`: the obs-owned default-deny line schemas for home-level process-log lines and instance detail lines. Each is self-contained (no cross-file `$ref`), and both are pinned by `tests/contract_diag_schema.rs`.
- `crates/viola-core/proptest-regressions/`: committed proptest failure seeds.
- `fuzz/`: the separate cargo-fuzz workspace (`viola-fuzz`, `[package.metadata] cargo-fuzz = true`, its own `[workspace]` and committed `Cargo.lock`). It holds `fuzz/rust-toolchain.toml` (the fuzz-only `channel = "nightly-2026-09-20"`), `fuzz/fuzz_targets/<target>.rs` (today `viola_name`) and `fuzz/corpus/<target>/` (committed synthetic seeds). `fuzz/target/`, `fuzz/artifacts/` and `fuzz/coverage/` are gitignored; the nightly `fuzz` job uploads `fuzz/artifacts/` on failure.
- `target/agent-run/<session>/` (harness session record, supervisor handshake files, the session's `bin/claude[.exe]` copy), `target/agent-run/chunk.diff` and `target/agent-run/artifacts/`: written by `viola-harness`, gitignored. `chunk.diff` (the mutation leg's diff, repository source text) is the one file `secret-scan` skips, by exact path, and the `harness-<os>` upload excludes it. `artifacts/` holds the per-suite JUnit, `run-summary.json`, `llvm-cov-summary.json` (from `run --coverage`) and `mutants-verdict-<leg>.json` (from `run --mutants --leg`: repo-relative mutant names and outcomes only).
- `target/lcov.info`: the `run --coverage` LCOV output. Never uploaded, gitignored.
...
- `target/supply-chain/`: `deny.json` (the root graph), `deny-fuzz.json` (the `fuzz/Cargo.lock` advisories and sources audit) and `zizmor.json` from the CI `supply-chain` job, uploaded as artifact `supply-chain` (7 days, admissible by content: obs-plan §8 item 6), gitignored.
- `target/orphans-probes/run-<utc>-<pid>/`: the throwaway crates `scripts/orphans-check.sh --probe` writes per run, each with an empty `[workspace]`: a planted `src/stray.rs`, a file gated on a target no CI triple compiles, and a `cfg(windows)`/`cfg(unix)` file-pair control (verdict `orphans-check probes: 2/2 fired, control clean`), gitignored.
- `target/release-check/`: the cargo target dir the local release-check gate entry passes as `CARGO_TARGET_DIR` (the script honours an inherited one and never sets it), gitignored.
- `target/perf/`: the `run --perf` target dir (`cargo build --release --workspace --features fake-agent --target-dir target/perf`, binaries under `target/perf/release`; test-plan §10), gitignored. Its hyperfine exports are the separate `perf/*.json` (obs-plan §9).
- `target/nextest/ci/junit.xml`: the nextest `ci` profile's JUnit, uploaded as `junit-<os>` behind the secret scan, gitignored.
- `e2e-web/test-results/`: the Playwright suite's outputs, gitignored; they land with the Web UI chunks. `e2e-web/test-results/a11y/` holds the a11y fixture's violation rows (`<test-id>.ndjson`, validated against `e2e-web/schemas/a11y-row.v1.json`), and `e2e-web/test-results/lint/` holds the eslint-plugin-lit-a11y and html-validate reports.
...
- `target/tools/ripgrep/bin/`: ripgrep 15.2.0, installed by `scripts/install-ripgrep.sh`, gitignored. `target/tools/ripgrep/probe-*`: the scratch dirs of `scripts/install-ripgrep.sh --probe` (TLS failure retried then fail-closed, digest mismatch refused, matching digest passes; no network), gitignored.
...
- Code-graph planes (`scripts/code-graph.py`, one DuckDB per language plane under `.andromeda/cache/{plane}/`):
  - **rust:** the root workspace through rust-analyzer SCIP. It indexes every member (`viola`, `viola-core`, `viola-pty`, `viola-agent-claude`, `viola-e2e`). `fuzz/`, a separate workspace, is outside it.
...
- Licence: the project is `MIT OR Apache-2.0` (founder ruling 2026-09-25). `license` is set once in root `[workspace.package]` and inherited with `license.workspace = true` by `viola`, `viola-core`, `viola-pty`, `viola-agent-claude` and `viola-e2e`; `fuzz/Cargo.toml`, its own workspace, carries the literal. The texts are `LICENSE-MIT` (holder Turbolet85) and `LICENSE-APACHE` at the root, and `README.md` names both.
...
- `viola-pty` depends on no viola crate and knows no agent. Third-party (as landed): portable-pty, windows-sys (Windows only), libc (Unix only; termios raw mode and the terminal size). No thiserror ([Error Handling]).
- `viola-channel` → `viola-core`, interprocess, windows-sys (Windows only: the SQOS client open adopted by `Stream::try_from`). It has a sync client and server, plus a Tokio client behind the `tokio` feature.
...
- `viola-agent-claude` → `viola-core`, `viola-state`, serde_path_to_error (drift reports on external payloads), vt100 (the screen model and signatures behind the readiness gate; `run`'s pump feeds it bytes). This is the only crate that knows Claude payload shapes, including `claude agents --json`. As landed it depends on thiserror only (`IDENTITY_FLOOR`, `plan_strip`, `resolve_program`, `Refusal`); `viola-core`, `viola-state`, serde_path_to_error and vt100 arrive with the chunks that consume them.
...
- `viola-e2e` (test-only; no product crate depends on it) takes syn and proc-macro2 (`span-locations`) for `harness::cfg_legs`, the mutation union's `#[cfg]` reader; its other dependencies are not listed here.
- The `viola` root bin → all members, clap, anyhow, and windows-sys (Windows only: the registry reader for the R8 persistent-environment names in `src/run/env.rs`). It contains the subcommand dispatch, the `run` pump, the wheel and the budget governor, all of which consume only normalised events.
...
├── schemas/                    # JSON schemas: fake-script.v1.json (test-side), diag-line/diag-detail.v1.json (obs line contracts)
├── crates/
...
│   ├── release-check.sh        # target job 6: the release build carries `viola` only (+ --probe)
...
├── e2e-web/                    # test-side Node only (Playwright + axe + the a11y lint); the ts code-graph plane
│   ├── package.json            # pins @playwright/test and @axe-core/playwright
...
│   ├── tests/*.spec.ts         # one spec per bay layout type
...
├── .github/
│   └── workflows/
│       ├── ci.yml              # push + PR: 3-OS test/lint (+ module orphans), mutants legs + verdict, msrv,
│       │                       #   fuzz-replay, 3-OS release (release-check), supply-chain (+ fuzz lockfile audit)
│       └── nightly.yml         # weekly schedule + workflow_dispatch: cargo deny check advisories (root + fuzz/Cargo.lock) + fuzz time-box
├── refs/                       # brief and prior-art survey (arch input)
└── .andromeda/                 # pipeline runs and cache
```

**CI/CD approach**
- GitHub Actions, two workflows. `ci.yml`, triggered on push and pull request, with matrix `os: [windows-2025, macos-latest, ubuntu-latest]` on native runners, is the only push/PR pipeline. `nightly.yml`, triggered by a weekly `schedule` and `workflow_dispatch` (both fire from the repository's default branch), runs two ubuntu jobs with no cache. `advisories` runs `cargo deny check advisories` over the root lockfile and `cargo deny --manifest-path fuzz/Cargo.toml check advisories` over the fuzz lockfile, because the advisory DB moves without code changes. `fuzz` runs `cargo +<channel> fuzz run --fuzz-dir fuzz <t> fuzz/corpus/<t> -- -max_total_time=120` per target (the channel comes from `fuzz/rust-toolchain.toml`) and uploads `fuzz/artifacts/` on `failure()`. Neither workflow has a `concurrency:` block. zizmor's pedantic `concurrency-limits` was declined because a concurrency group cancels pending runs, which would drop a cancelled run's `always()` gate and upload chain. (Every mutation run now covers the whole chunk from the harness-derived base, so a cancelled run no longer loses mutation coverage; that half of the original reason is retired.)
...
- Target jobs per OS, each wired by the chunk that owns it (Supply-chain gates, Quality gates, Workspace tree):
  ...
  6. `cargo build --release --locked --bin viola` through `scripts/release-check.sh` (the `release` job): it judges that build's own `compiler-artifact` executables, fails on any other than `viola` (`viola-harness`, `viola-fake-agent`) or on none, and never lists `target/release/`, where a shared or cached dir keeps stale test-only exes (measured at chunk 2026-09-24-workspace-tree-and-code-graph-planes: a bare `cargo build --release` and `--bin viola` yield `viola` only, `--workspace` adds `viola-harness`)
...
- **Config management.** Precedence: CLI flags override `<viola home>/config.json`, which overrides built-in defaults.
  ...
  - Settings: budget thresholds as `Percent` (defaults `five_hour` 90, `seven_day` 85), the GUI port (default 47319), `diagnostics_level` (`info` by default, or `debug`) and `claude_env_keep` (the Unix R8 pass-list of `CLAUDE*` names, default empty). `diagnostics_level` is the only source of the process-log level; `RUST_LOG` has no effect.
...
- **Mixed-version tolerance.** Every channel `params`, event, snapshot and config carries `v`, and channel `params` also carry the sender's `CARGO_PKG_VERSION`. Readers skip unknown kinds and fields and surface the counts. A newer request than the wrapper supports gets a `-32602` naming the wrapper's version.
...
- **Diagnostic output channels.** `hook` writes only its decision body to stdout and nothing to stderr (Hook Contract). While the child runs, `run` writes nothing to the terminal except the child's own output, because the terminal carries the child's screen. Every role's codes-only process log goes to its home-level role file under `diagnostics/`: `run-<name>`, `hook-<name>`, `mcp`, `ui-<port>` or `cli-<name>` `.ndjson`. Content-bearing detail belongs only in the instance's `diagnostics/detail-<role>.ndjson`. `mcp` writes only MCP frames to stdout, and its log goes to `diagnostics/mcp.ndjson`; it writes JSON to stderr only when that file cannot be opened. `ui` and short-lived CLI verbs may use stderr for their human output. The logger and the format are owned by obs.
...
- Errors: thiserror 2.0.20 per crate (except `viola-pty`'s hand-written `PtyError`), anyhow 1.0.104 only in the root bin; typed CLI exit codes; `viola hook` always exits 0.
...
- Publishability: every workspace crate, including the root `viola` bin, sets `publish = false` and inherits `license = "MIT OR Apache-2.0"` from `[workspace.package]`. Nothing is published to crates.io in v1.
