...
- 2026-09-24: Stopping a Monitor/background task leaves its `tail.exe`/`grep.exe` running and holding their files open, which blocks renames of that directory (cargo-mutants' `mutants.out` → `.old`): watch tool output with a handle-free poll (`cat` per interval), never `tail -F`, and stop leftovers by verified pid. Extended 2026-09-26: the session's rust-analyzer holds `mutants.out` the same way and restarts after every edit, so stop it by exact `ExecutablePath` immediately before each `run --mutants`.
