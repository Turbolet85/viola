...
- 2026-09-24: Keep a `#[cfg(unix)]`-only function to a minimal OS reader, and put its decision logic in a plain function tested on every OS. A Windows host cannot compile, so cannot kill, mutants of a unix-only body; they are killable only on the Linux CI mutants job.
