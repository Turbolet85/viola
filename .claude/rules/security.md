...
## Dependencies and CI
- **Audit tool:** `cargo deny check` (>=0.20.2: advisories, bans, licences, sources — crates.io only; the only ignore is RUSTSEC-2017-0008 via portable-pty `=0.8.1`); the tokio ban is `deny-sync.toml`, run per sync crate as sole root; `scripts/deny-probes.sh` proves every ban fires; weekly `cargo deny check advisories` in `nightly.yml` over the root and fuzz lockfiles; `zizmor` on workflows. Never widen an ignore, add a `skip`/`allow` or silence zizmor to reach green.
