...
## Dependencies and CI
- **Audit tool:** `cargo deny check` (>=0.20.2: advisories, bans, licences, sources — crates.io only; the only ignore is RUSTSEC-2017-0008 via portable-pty `=0.8.1`); weekly scheduled `cargo deny check advisories`; `zizmor` on workflows.
