...
- Internal (forwarded by the shims, not agent-facing): `supervise`, `ui-restart --session <id>`, `gate --require <suites> [--artifacts <dir>] [--mutants-legs a,b]` (the per-job verdict; a missing artifact is a breach; with `--mutants-legs` each mutant is judged only by the legs whose `#[cfg]`s compile its line — read from the checked-out source, every leg when none does — and is red when no judged leg caught it and some judged leg missed it or timed out; a leg with more unviable than caught mutants is already red at its own run)
