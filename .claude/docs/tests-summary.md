...
| Mutation | 0 missed, 0 timeout, unviable ≤ caught in the chunk diff | cargo-mutants 27.1.0 `--in-diff`; more unviable than caught mutants is red (`unviable-exceeds-caught`: the builds failed, nothing was tested); a Rust delta reads a fresh `outcomes.json` (`verdict:"counted"`), a diff with no `.rs` path passes as `verdict:"no-rust-delta"`, and one whose `.rs` paths are all test targets (`tests/`, `benches/`, `examples/`) as `verdict:"test-only-rust-delta"` (cargo-mutants mutates none of them); a mixed diff is `counted`. CI runs two legs (ubuntu-latest, windows-2025) over the whole chunk (a harness-derived base) and gates their union (`gate --mutants-legs`): each mutant is judged only by the legs whose `#[cfg]`s compile its line (every leg when none does) and is red when no judged leg caught it and some judged leg missed it, because cargo-mutants reports another OS's `#[cfg]` bodies as missed |
